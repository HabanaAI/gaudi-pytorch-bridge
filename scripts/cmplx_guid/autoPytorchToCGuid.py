...
for line in lines:
    ...
    if word1[0] == "#": continue    # comment
    elif word1 == "npu_stack_path": path_to_npu_stack = word2 # Path to npu_stack - /home/tkanvar/trees/npu-stack
    elif word1 == "registration_declaration_path": path_to_reg_decl = word2 # Path to registration declarations file - /home/tkanvar/anaconda3/envs/py_venv_3.8/lib/python3.8/site-packages/torch/include/ATen/RegistrationDeclarations.h
    elif word1 == "hpu_op_yaml_entry": hpu_op = word2 # ops in hpu_op.yaml, comma separated if multiple ops have same op_backend
    elif word1 == "reg_decl_op": reg_decl_op = word2 # op name in registraion declaration file
    elif word1 == "op_name": op = word2 # frac
    elif word1 == "params": params = word2 # ns_QuadTree::ParamsSegmentsPerAxis, if no param then NoParam
    elif word1 == "trait": trait = word2 # BroadcastableElementwiseTrait
    elif word1 == "kernel_op": kernel_op = word2 # FracKernelOp
    elif word1 == "op_folder": op_folder = word2 # Elementwise
    elif word1 == "op_file_name": op_file_name = word2 # Frac, which will be made FracTest.cpp and FracTest.hpp
    elif word1 == "inputs_dim_max": inputs_dim_max = word2 # max_dim_of_inputs_in_order_of_op_defintion_including_optional_comma_separated -- If no limit then write MAX_TENSOR_DIM
    elif word1 == "inputs_dim_min": inputs_dim_min = word2 # min_dim_of_inputs_in_order_of_op_defintion_including_optional_comma_separated[p0] 
    elif word1 == "outputs_dim_max": outputs_dim_max = word2
    elif word1 == "outputs_dim_min": outputs_dim_min = word2
    elif word1 == "torch_op": torch_op = word2 # torch::frac
    elif word1 == "pbtxt_file_path": pbtxt_file_path = word2 # /home/tkanvar/trees/npu-stack/tpc_kernels/doc/training/op_def/07_misc/frac.pbtxt
    elif word1 == "greco_pbtxt_file_path": greco_pbtxt_file_path = word2 # /home/tkanvar/trees/npu-stack/tpc_kernels/doc/inference/op_def/elementwise/unary/frac.pbtxt
    elif word1 == "is_fwd": 
        if word2.lower() == "true": is_fwd = True # if the op provided have fwd entry apart from normal entry, then append in preexisting cpp, hpp files.
    elif word1 == "is_bwd": 
        if word2.lower() == "true": is_bwd = True  # if the op provided has bwd entry, then append in preexisting cpp, hpp files
...
op_file = path_to_npu_stack + "/tpc_fuser/mlir/lib/Optimizer/Transforms/ComplexGuid/" + op_folder + "/" + op_file_name + ".cpp"
op_test_cpp_file = path_to_npu_stack + "/tpc_fuser/mlir/pytenettests/ComplexGuidTests/" + op_folder + "/" + op_file_name + "Test.cpp"
op_test_hpp_file = path_to_npu_stack + "/tpc_fuser/mlir/pytenettests/ComplexGuidTests/" + op_folder + "/" + op_file_name + "Test.hpp"
...
if make_changes_in_folders.count("tpc_fuser") > 0 and (make_changes_in_file == "" or make_changes_in_file == td_file):
        ...
        for line in lines:
            ...
            if (fn[1] == op or (len(fn) > 2 and fn[2] == op)):
                ...
                for p in params_schema:
                    ...
                    shape = "Shape"
                    if num_inputs > len(inputs_dim_max): break

                    if inputs_dim_max[num_inputs-1] == inputs_dim_min[num_inputs-1]: shape = "D" + str(inputs_dim_max[num_inputs-1])

                    any_match = "Any"
                    if inputs_dim_max[num_inputs-1] == outputs_dim_max[0] and inputs_dim_min[num_inputs-1] == outputs_dim_min[0]: 
                        any_match = "Match"
                        isInputMatch = True
                    inputs_any_match.append(any_match)

                    if len(p.split("=")) > 1:
                        if inputs == "": inputs += any_match + shape + "<Optional<PrecisionType>>"
                        else: inputs += ", " + any_match + shape + "<Optional<PrecisionType>>"
                        op_info["is_optional"] = True
                        op_info["optional_inp"].append(1)
                        prm = p.split("=")[1]
                        if prm.find(")") != -1: prm = prm.split(")")[0]
                        op_info["optional_inp_def_val"].append(prm)
                    else:
                        if inputs == "": inputs += any_match + shape + "<PrecisionType>"
                        else: inputs += ", " + any_match + shape + "<PrecisionType>"
        ...
        for line in lines:
            ...
            if is_op_found:
                ...
                if feature_line[0] == "dtypes":               
                    ...
                    is_first = True
                    for d in dtypes_hpu:
                        if not is_first: features_map["dtypes"] += ", "
                        is_first = False
                        if d == "BFloat16": features_map["dtypes"] += "BF16"
                        elif d == "Float": features_map["dtypes"] += "F32"
                        elif d == "Int": features_map["dtypes"] += "I32"
    ...
    if outputs_dim_max[0] == outputs_dim_min[0]: outputs_str += str(outputs_dim_max[0])
    else: outputs_str += "Shape"
    ...
    if is_fwd: op_td += ", " + op + "_fwd"
    op_td += "\"],\n"
    op_td += "    /* inputs=*/[" + inputs_str + "],\n"
    op_td += "    /*outputs=*/[" + outputs_str + "],\n"
    if params == "NoParams": op_td += "    " + params + ",\n"
    else: op_td += "    Params<\"" + params + "\">,\n"
    op_td += "    [" + trait + ",\n"
...
if make_changes_in_folders.count("tpc_fuser") > 0 and (make_changes_in_file == "" or make_changes_in_file == cmplx_guid_supp_list_file):
...
if make_changes_in_folders.count("tpc_fuser") > 0 and (make_changes_in_file == "" or make_changes_in_file == op_file):
    ...
    def writeHeader(file_buf):
        file_buf.write("//===- " + op_file_name + ".cpp -- " + op_file_name + " Op conversion ------------------------------===//\n")
        file_buf.write("//\n")
    ...
    def writeFn(file_buf, params, num_inputs, num_outputs, features_map, op_info):
        ...
        if params == "NoParams": file_buf.write("LogicalResult handleComplexGuid(KernelOp op, CommonNodeLoc loc, RewriterBase &rewriter) {\n")
        else: file_buf.write("LogicalResult handleComplexGuid(KernelOp op, " + params + "* params, CommonNodeLoc loc, RewriterBase &rewriter) {\n")
        ...
        for i in range(num_outputs):
            ...
            file_buf.write("\tauto resultType" + str(i) + " = SynTensorType::get(out" + str(i) + "Type.getMinShape(), out" + str(i) + "Type.getMaxShape(), precisionType);\n")
        ...
        def assignVar(line, str_if_not_equal1, str_if_not_equal2, addNode_new_var_iter):
            ...
            elif (str_if_not_equal2 == "" and line.find(str_if_not_equal1) != -1) or line.find(str_if_not_equal1) != -1 or (str_if_not_equal2 != "" and line.find(str_if_not_equal2) != -1):
        ...
        def get_kernelop(guid):
            ...
            for i in guid:
                if i == '_': 
                    is_first_char_of_word = True
                    continue
                if is_first_char_of_word: kernelop += i.upper()
                else: kernelop += i
        ...
        for line in gen_lines:
            ...
            if is_opbackend_present and line.find(features_map["op_backend"] + "::AddNode") != -1:
                ...
                if param_iter == 2:
                    addNode_guid = ""
                    if line.find("MULT_GUID") != -1: addNode_guid = "mult_"
                    else: addNode_guid = line.split("\"")[1]
            ...
            elif line.find("ConstantHelper(") != -1 or line.find("BuildConstant(") != -1 or constant_helper_found or build_constant_found:
                if line.find("ConstantHelper(") != -1: constant_helper_found = True
                elif line.find("BuildConstant(") != -1: build_constant_found = True
            
                addNode_new_var_iter, addNode_var = assignVar(line, "ConstantHelper(", "BuildConstant(", addNode_new_var_iter)
                ...
                if line.find(";") != -1:
                    ...
                    const_val = words[2]
                    if constant_helper_found: const_val = words[1]
                    op_info["params"].append("constant = " + const_val)
                    file_buf.write(const_val)
                    file_buf.write(";\n")
                    addNode_inputs = ["constantParams"+addNode_var]

            elif line.find("ReshapeHelper(") != -1 or line.find("BuildReshape(") != -1 or reshape_helper_found or build_reshape_found:
                if line.find("ReshapeHelper(") != -1: reshape_helper_found = True
                elif line.find("BuildReshape(") != -1: build_reshape_found = True

                addNode_new_var_iter, addNode_var = assignVar(line, "ReshapeHelper(", "BuildReshape(", addNode_new_var_iter)
                ...
                if line.find(";") != -1:
                    ...
                    if reshape_helper_found: addNode_shape = words[2]
                    else: addNode_shape = words[3]
                    if reshape_helper_found: addNode_inputs = [words[1], "nullptr"]
                    else: addNode_inputs = [words[2], "nullptr"]

                    op_info["params"].append("operand_segment_sizes = array<i32: 1, 0, 0>")

            elif line.find("BroadcastHelper(") != -1 or line.find("BuildBroadcast(") != -1 or build_broadcast_found or broadcast_helper_found:
                if line.find("BroadcastHelper(") != -1: broadcast_helper_found = True
                elif line.find("BuildBroadcast(") != -1: build_broadcast_found = True

                addNode_new_var_iter, addNode_var = assignVar(line, "BroadcastHelper(", "BuildBroadcast(", addNode_new_var_iter)
                ...
                if line.find(";") != -1:
                    ...
                    if broadcast_helper_found: addNode_shape = words[2]
                    else: addNode_shape = words[3]
                    if broadcast_helper_found: addNode_inputs = [words[1], "nullptr"]
                    else: addNode_inputs = [words[2], "nullptr"]
                    op_info["params"].append("")

            if line.find(";") != -1 and addNode_kernelOp != "":
                file_buf.write("\tauto newType_"+addNode_var+" = SynTensorType::get("+addNode_shape+", precisionType);\n")
                file_buf.write("\tValue "+addNode_var+" = rewriter.create<"+addNode_kernelOp+">(loc, newType_"+addNode_var+", precisionType, ")
                
                inputs_temp = []
                is_first = True
                for i in addNode_inputs:
                    if not is_first: file_buf.write(", ")
                    else: is_first = False
        ...
        file_buf.write("\n")
        if num_outputs == 1: file_buf.write("\trewriter.replaceOp(op, {result0});\n")
        else:
            for i in range(num_outputs):
        ...
        if params == "NoParams": file_buf.write("LogicalResult extractFunctionalComplexGuid(" + kernel_op + " op, CommonNodeLoc loc, RewriterBase &rewriter) {\n")
        else: file_buf.write("LogicalResult extractFunctionalComplexGuid(" + kernel_op + " op, " + params + "* params, CommonNodeLoc loc, RewriterBase &rewriter) {\n")
        ...
        for i in range(num_outputs):
            ...
            if outputs_dim_max[i] == MAX_TENSOR_DIM:
                ...
                if params == "NoParams": file_buf.write("\t\treturn performNDOperation<"+kernel_op+">(op, nullptr, loc, rewriter);")
                else:  file_buf.write("\t\treturn performNDOperation<"+kernel_op+">(op, params, loc, rewriter);")
                file_buf.write("\n")

        if params == "NoParams": file_buf.write("return handleComplexGuid<"+kernel_op+">(op, loc, rewriter);\n")
        else: file_buf.write("\treturn handleComplexGuid<"+kernel_op+">(op, params, loc, rewriter);\n")
        file_buf.write("}\n\n")
        ...
        if is_fwd:
            ...
            if params == "NoParams": file_buf.write("LogicalResult extractFunctionalComplexGuid(" + kernel_op_fwd + " op, CommonNodeLoc loc, RewriterBase &rewriter) {\n")
            else: file_buf.write("LogicalResult extractFunctionalComplexGuid(" + kernel_op_fwd + " op, " + params + "* params, CommonNodeLoc loc, RewriterBase &rewriter) {\n")
            ...
            for i in range(num_outputs):
                ...
                if outputs_dim_max[i] == MAX_TENSOR_DIM:
                    file_buf.write("\tif (out"+str(i)+"Rank > MAX_TENSOR_DIM)\n")
                    if params == "NoParams": file_buf.write("\t\treturn performNDOperation<"+kernel_op_fwd+">(op, nullptr, loc, rewriter);")
                    else:  file_buf.write("\t\treturn performNDOperation<"+kernel_op_fwd+">(op, params, loc, rewriter);")
                    file_buf.write("\n")

            if params == "NoParams": file_buf.write("return handleComplexGuid<"+kernel_op_fwd+">(op, loc, rewriter);\n")
            else: file_buf.write("return handleComplexGuid<"+kernel_op_fwd+">(op, params, loc, rewriter);\n")
            file_buf.write("}\n\n")
...
if make_changes_in_folders.count("tpc_fuser") > 0 and (make_changes_in_file == "" or make_changes_in_file == tpckernel_ops_file):
    ...
    for i in range(num_inputs):
        ...
        for j in range(inputs_dim_min[i]):
            if not is_first1: op_test += "x"
            is_first1 = False

            val = ""
            if (inputs_any_match[i] == "Match" and len(match_dims) != 0): val = match_dims[j]
            else: 
                val = random.randint(10,20)
            if inputs_any_match[i] == "Match": match_dims.append(val)
    ...
    for i in range(num_outputs):
        ...
        for j in range(outputs_dim_min[i]):
            ...
            val = ""
            if outputs_any_match[i] == "Match": val = match_dims[j]
            else: val = random.randint(10,20)
    ...
    for i in range(len(inputs_tensor)):
        if not is_first: repeat_line += ", "
        is_first = False

        repeat_line += "tensor<"
        is_first1 = True
        for j in range(len(inputs_tensor[i])):
            if not is_first1: repeat_line += "x"
            is_first1 = False
...
if make_changes_in_folders.count("tpc_fuser") > 0 and (make_changes_in_file == "" or make_changes_in_file == complex_guid_file):
    ...
    if op_info["is_optional"]:
        ...
        for i in range(num_inputs): op_test += "1, "
        op_test += "0>}"
    ...
    for i in range(len(op_info["outputs_var"])):
        ...
        if op_info["params"][i] != "": op_test += " {" + op_info["params"][i] + "}"

        op_test += " : <ToDo: Please fill tensors accordingly>\n"
        
    op_test += "// CHECK-NEXT:  syn_rt.exit "
    if len(op_info["actual_outputs_var"]) > 0:
        op_test += op_info["actual_outputs_var"][len(op_info["actual_outputs_var"]) - 1] + " : <ToDo: Please fill tensors accordingly>\n"
    else:
        op_test += "<ToDo: Please fill tensors accordingly>\n"
    op_test += "\n"
    op_test += "\n"
...
######## Populate opTest.hpp
if make_changes_in_folders.count("tpc_fuser") > 0 and (make_changes_in_file == "" or make_changes_in_file == op_test_hpp_file):
    print ("Writing in file = ", op_test_hpp_file)

    op_test_hpp_file_buf = open(op_test_hpp_file, "w")

    op_test_hpp_file_buf.write("//===--------- "+op_file_name+"Test.hpp - "+op_file_name+" ComplexGuid Test ---------===//\n")
    ...
    op_test_hpp_file_buf.write("\n")
    op_test_hpp_file_buf.write("enum dim {\n  oneDim = 1,\n  twoDim,\n  threeDim,\n  fourDim,\n  fiveDim,\n  sixDim,\n  sevenDim,\n  eightDim,\n  nineDim\n};\n")
    op_test_hpp_file_buf.write("\n")
    op_test_hpp_file_buf.write("template <const synDataType inType = syn_type_float,\n")

    def getDim(num):
        if num == 1: return "oneDim"
        elif num == 2: return "twoDim"
        elif num == 3: return "threeDim"
        elif num == 4: return "fourDim"
        elif num == 5: return "fiveDim"
        elif num == 6: return "sixDim"
        elif num == 7: return "sevenDim"
        elif num == 8: return "eightDim"
        elif num == 9: return "nineDim"
    ...
    if op_info["is_optional"]: inTensorDim += 1
    ...
    for i in range(inTensorDim):
        if not is_first: op_test_hpp_file_buf.write(", ")
        is_first = False
    ...
    is_first = True
    for i in range(inTensorDim):
        if not is_first: op_test_hpp_file_buf.write(" << \"x\" << ")
        is_first = False
    ...
    for i in range(len(op_info["optional_inp"])):
        ...
        if op_info["optional_inp"][i]:
            ...
            ifmInitializer_count += 1
            if op_info["is_optional"]: op_test_hpp_file_buf.write("  unsigned int ifmInitializer"+str(i)+"["+getDim(inTensorDim-1)+"];\n")
            else: op_test_hpp_file_buf.write("  unsigned int ifmInitializer"+str(i)+"["+getDim(inTensorDim)+"];\n")

    if op_info["is_optional"]: op_test_hpp_file_buf.write("  unsigned int ofmInitializer["+getDim(inTensorDim-1)+"];\n")
    else: op_test_hpp_file_buf.write("  unsigned int ofmInitializer["+getDim(inTensorDim)+"];\n")
    op_test_hpp_file_buf.write("\n")
    ...
    for j in range(len(op_info["optional_inp"])):
        if op_info["optional_inp"][j] == 0:
            if inputs_dim_max[j] == inputs_dim_min[j]:
                for i in range(inputs_dim_max[j]): op_test_hpp_file_buf.write("    ifmInitializer"+str(j)+"["+str(i)+"] = ::testing::get<"+str(i)+">(TestParam());\n")
            else: 
                itd = inTensorDim
                if op_info["is_optional"]: itd -= 1
                for i in range(itd): op_test_hpp_file_buf.write("    ifmInitializer"+str(j)+"["+str(i)+"] = ::testing::get<"+str(i)+">(TestParam());\n")
        else:
            op_test_hpp_file_buf.write("    ifmInitializer"+str(j)+" = ::testing::get<"+str(inTensorDim-1)+">(TestParam());\n")
    ...
    is_first = True
    for i in range(len(op_info["optional_inp"])):
        if op_info["optional_inp"][i] == 0: continue
    ...
    for i in range(len(optional_params_permutation)):
        ...
        for j in range(len(optional_ifm_var)):
            if found_without and optional_params_permutation[i][j] == "w": 
                ...
                break
            if optional_params_permutation[i][j] == "w/o": found_without = True
        if need_to_contnue: continue
        ...
        if is_first: 
            ...
            op_test_hpp_file_buf.write(" ("+optional_ifmInitializer_var+" == "+str(i)+") {\n")
        else: op_test_hpp_file_buf.write("     } else {\n")
        is_first = False

        for j in range(len(optional_ifm_var)):
            if optional_params_permutation[i][j] == "w/o": op_test_hpp_file_buf.write("      "+optional_ifm_var[j]+" = nullptr;\n")
            else: 
                op_test_hpp_file_buf.write("      ifm"+str(j)+"_1 = createDeviceTensor(\"Optional"+str(j)+"\", INPUT_TENSOR, MEM_INIT_RANDOM_POSITIVE, nullptr, scalarType, dim, optional_inp"+str(j)+");\n")
                op_test_hpp_file_buf.write("      ifm"+str(j)+" = &ifm"+str(j)+"_1;\n")

    if op_info["is_optional"]: op_test_hpp_file_buf.write("     }\n")
    op_test_hpp_file_buf.write("    // Create device input/output tensors.\n")

    for i in range(len(op_info["optional_inp"])):
        if op_info["optional_inp"][i] == 1: break
        
        itsrdim = MAX_TENSOR_DIM
        if inputs_dim_min[i] == inputs_dim_max[i]: itsrdim = inputs_dim_max[i]

        if (op_info["is_optional"] and itsrdim == inTensorDim-1) or (not op_info["is_optional"] and itsrdim == inTensorDim):
            op_test_hpp_file_buf.write("    ifm"+str(i)+" = createDeviceTensor(\"A"+str(i)+"\", INPUT_TENSOR, MEM_INIT_ALL_ONE, nullptr, inType, inTensorDim, ifmInitializer"+str(i)+");\n")
        else:
            op_test_hpp_file_buf.write("    ifm"+str(i)+" = createDeviceTensor(\"A"+str(i)+"\", INPUT_TENSOR, MEM_INIT_ALL_ONE, nullptr, inType, "+getDim(itsrdim)+", ifmInitializer"+str(i)+");\n")

    op_test_hpp_file_buf.write("    ofm = createDeviceTensor(\""+op_file_name+"\", OUTPUT_TENSOR, MEM_INIT_ALL_ZERO, nullptr, outType, outTensorDim, ofmInitializer);\n")
    op_test_hpp_file_buf.write("\n")
    ...
    is_first = True
    for i in op_dtype:
        if is_first: op_test_hpp_file_buf.write("    if ")
        else:  op_test_hpp_file_buf.write("    } else if ")
        is_first = False
    ...
    for i in range(len(optional_params_permutation)):
        ...
        is_if_elif = True
        if is_first: op_test_hpp_file_buf.write("    if (")
        elif not is_first and i < len(optional_params_permutation)-1: op_test_hpp_file_buf.write("    else if (")
        else: 
        ...
        for j in range(len(optional_ifm_var)):
            if not is_first1: 
                if is_if_elif: op_test_hpp_file_buf.write(" && ")
            is_first1 = False

            if optional_params_permutation[i][j] == "w": 
                if is_if_elif: op_test_hpp_file_buf.write(optional_ifm_var[j] + " != nullptr")
    ...
    op_test_hpp_file_buf.write("    unsigned int ofmRef = createReferenceTensor(\""+op_file_name+"Ref\", outType, outTensorDim, ofmInitializer);\n")
    op_test_hpp_file_buf.write("\n")
    ...
    if not op_info["is_optional"]:
        ...
        for j in range(len(op_info["optional_inp"])):
            if not is_first1: op_test_hpp_file_buf.write(", ")
            is_first1 = False
    ...
    for i in range(len(optional_params_permutation)):
        ...
        for j in range(len(optional_ifm_var)):
            ...
            if optional_params_permutation[i][j] == "w/o": op_test_hpp_file_buf.write(" == nullptr")
            else: op_test_hpp_file_buf.write(" != nullptr")
        op_test_hpp_file_buf.write(") {")
        ...
        op_test_hpp_file_buf.write("\n      pytenetTensors[ofmRef].torchTensor = "+torch_op+"(")
        for j in range(len(op_info["optional_inp"])):
            if not is_first1 and (op_info["optional_inp"][j] == 0 or (op_info["optional_inp"][j] == 1 and optional_params_permutation[i][j-num_non_optional_inputs] == "w")) : 
                op_test_hpp_file_buf.write(", ")
            is_first1 = False
    ...
    op_test_hpp_file_buf.write("\n")
    op_test_hpp_file_buf.write("    ASSERT_TRUE(validateResults(pytenetTensors[ofmRef].torchTensor.data_ptr(), hostBuffers[ofm], tensorDescs[ofmRef].numElements, pdt));\n")
    op_test_hpp_file_buf.write("  }\n")
    op_test_hpp_file_buf.write("};\n")
    op_test_hpp_file_buf.write("\n")
    op_test_hpp_file_buf.write("} // namespace ComplexGuidTest\n")
...
######## Populate opTest.cpp
if make_changes_in_folders.count("tpc_fuser") > 0 and (make_changes_in_file == "" or make_changes_in_file == op_test_cpp_file):
    print ("Writing in file = ", op_test_cpp_file)

    op_test_cpp_file_buf = open(op_test_cpp_file, "w")

    op_test_cpp_file_buf.write("//===--------- "+op_file_name+"Test.cpp - "+op_file_name+" ComplexGuid Test ---------===//\n")
    ...
    def getDtypeName(dt):
        dt = dt.lower()
        if dt == "f32": return "syn_type_float"
        elif dt == "bf16": return "syn_type_bf16"
        elif dt == "f16": return "syn_type_f16"
        elif dt == "i32": return "syn_type_int"
            ...
            if not op_info["is_optional"]:                   # No options
                ...
                for k in range(len(optional_params_permutation)):
                    ...
                    for l in range(len(optional_params_permutation[k])):
                        if optional_params_permutation[k][l] == "w": testname += "WI" + str(l)
                        else: testname += "WoI" + str(l)

                    d = []
                    for m in range(inTensorDim-1):
                        if m >= inTensorDim_test[i]: d.append(0)
                        else:
                    ...
                    test_cases["class_name"].append(op_file_name + op_dtype[j].upper() + str(inTensorDim_test[i]) + "Test" + testname)
                    test_cases["dtype"].append(getDtypeName(op_dtype[j]))
                    test_cases["dimType"].append(getDim(inTensorDim_test[i]))
                    test_cases["dimTypeShort"].append(op_dtype[j].lower())

    for i in range(len(test_cases["class_name"])):
        op_test_cpp_file_buf.write("class " + test_cases["class_name"][i] + " : public " + op_file_name + "Test<" + test_cases["dtype"][i] + ", " + test_cases["dimType"][i] + ", " + test_cases["dimType"][i] + "> {};\n")
        op_test_cpp_file_buf.write("TEST_P("+test_cases["class_name"][i]+", "+op+"_"+test_cases["dimTypeShort"][i]+") { RunTest(); }\n")
        op_test_cpp_file_buf.write("INSTANTIATE_TEST_SUITE_P(\n")
        op_test_cpp_file_buf.write("    sanity, "+test_cases["class_name"][i]+",\n")
        op_test_cpp_file_buf.write("    ::testing::Combine(\n        ::testing::Values(synDeviceGaudi, synDeviceGaudi2, synDeviceGreco),\n")
        op_test_cpp_file_buf.write("        ::testing::Values(std::make_tuple(")

        is_first = True
        for j in range(inTensorDim):
            if not is_first: op_test_cpp_file_buf.write(", ")
            is_first = False
...
if make_changes_in_folders.count("tpc_fuser") > 0 and (make_changes_in_file == "" or make_changes_in_file == cmakelist_file):
    ...
    for line in lines:
        ...
        if line.split("/")[0].lower().replace(" ", "") > op_folder.lower() or (line.split("/")[0].lower().replace(" ", "") == op_folder.lower() and line.split("/")[1] > (op_file_name + ".cpp")):
            break
...
if make_changes_in_folders.count("tpc_fuser") > 0 and (make_changes_in_file == "" or make_changes_in_file == shape_inference_file):
    ...
    entry_line_indx = -1
    for line in lines:
        if not fn_found: fn_line_indx += 1
        if not entry_found: entry_line_indx += 1

        if found1 and not fn_found:
            fn_name = line.split("GetShapeInference")

            if len(fn_name) > 1: fn_name = fn_name[1].split("(")[0]
            else: 
                found1 = False
                continue
        ...
        if line.find("static ShapeInferenceReturn_t") != 1: found1 = True
    ...
    if op_info["is_optional"]:
        ...
        for i in range(len(op_info["optional_inp"])):
            if op_info["optional_inp"][i] == 1: 
                ...
                continue
            min_num_inputs += 1
        fn_lines += "  if (inputParams->inputTensorsNr < "+str(min_num_inputs)+" || inputParams->inputTensorsNr > "+str(max_num_inputs)+") {\n"
    else:
    ...
    if is_fwd: final_fn_lines += fn_lines_fwd_header + fn_lines

    # insert entry
    entry_lines = "        {\""+op+"\", GetShapeInference"+op_file_name+"},\n"
    if is_fwd: entry_lines += "        {\""+op+"_fwd\", GetShapeInference"+op_file_name+"Fwd},\n"
...
if make_changes_in_folders.count("pytorch-integration") > 0 and (make_changes_in_file == "" or make_changes_in_file == hpu_yaml_file):
    ...
    op_name_found = ""
    line_indx = -1
    lines_to_insert = []            # Array of arrays. 1st element in array of arrays - line #, 2nd elemet in array of arrays - "add" or "delete", 3rd element in array of arrays - line to add
...
if make_changes_in_folders.count("pytorch-integration") > 0 and (make_changes_in_file == "" or make_changes_in_file == gen_file):
...
if make_changes_in_folders.count("tpc_kernels") > 0 and (make_changes_in_file == "" or make_changes_in_file == greco_pbtxt_file_path):
    ...
    is_first = True
    for i in op_dtype:
        if not is_first: greco_pbtxt_file_path_buf.write(", ")
        is_first = False

        if i == "F32": greco_pbtxt_file_path_buf.write("tensor(float32)")
        elif i == "BF16": greco_pbtxt_file_path_buf.write("tensor(bfloat16)")
        elif i == "F16": greco_pbtxt_file_path_buf.write("tensor(float16)")
        elif i == "I32": greco_pbtxt_file_path_buf.write("tensor(int32)")
    ...
    greco_pbtxt_file_path_buf.write("  }\n")
    greco_pbtxt_file_path_buf.write("  summary: \"The output is calculated as, O = <ToDo: Write equation of op using above input variables>\"\n")
    greco_pbtxt_file_path_buf.write("  description: \"<ToDo: Write description\\n\"\n")
    greco_pbtxt_file_path_buf.write("  constraints: \"Accepts "+str(num_inps)+" input tensors, with "+str(num_optional_inps)+" optional, and one output tensor.\\n\"\n")
    ...
    for i in op_dtype:
        ...
        if i == "F32": greco_pbtxt_file_path_buf.write(op+"_f32")
        elif i == "BF16": greco_pbtxt_file_path_buf.write(op+"_bf16")
        elif i == "F16": greco_pbtxt_file_path_buf.write(op+"_f16")
        elif i == "I32": greco_pbtxt_file_path_buf.write(op+"_i32")
    greco_pbtxt_file_path_buf.write("]\n")
...
if make_changes_in_folders.count("tpc_kernels") > 0 and (make_changes_in_file == "" or make_changes_in_file == pbtxt_file_path):
    ...
    is_first = True
    for i in op_dtype:
        if not is_first: pbtxt_file_path_buf.write(", ")
        is_first = False

        if i == "F32": pbtxt_file_path_buf.write("tensor(float32)")
        elif i == "BF16": pbtxt_file_path_buf.write("tensor(bfloat16)")
        elif i == "F16": pbtxt_file_path_buf.write("tensor(float16)")
        elif i == "I32": pbtxt_file_path_buf.write("tensor(int32)")
    ...
    pbtxt_file_path_buf.write("  }\n")
    pbtxt_file_path_buf.write("  summary: \"The output is calculated as, O = <ToDo: Write equation of op using above input variables>\"\n")
    pbtxt_file_path_buf.write("  description: \"<ToDo: Write description\\n\"\n")
    pbtxt_file_path_buf.write("  constraints: \"Accepts "+str(num_inps)+" input tensors, with "+str(num_optional_inps)+" optional, and one output tensor.\\n\"\n")
    ...
    for i in op_dtype:
        ...
        if i == "F32": pbtxt_file_path_buf.write(op+"_f32")
        elif i == "BF16": pbtxt_file_path_buf.write(op+"_bf16")
        elif i == "F16": pbtxt_file_path_buf.write(op+"_f16")
        elif i == "I32": pbtxt_file_path_buf.write(op+"_i32")
    pbtxt_file_path_buf.write("]\n")
