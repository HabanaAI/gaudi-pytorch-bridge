...
def frontend(
    ...
    if dtypes:
        ...
        if ctxop.supports_type_promotion() or ctxop.promote_int_to_float():
            # Exclude out tensor
            params = param_vars[:-1] if is_out_fn(fname) else param_vars[:]
            ...
            code += (
                f"  auto&& compute_type = "
                f"DTypeHelper::get_compute_dtype({{{', '.join(params)}}}, "
                f"c10::nullopt, "
                f"{str(promote_to_common_type).lower()}/*promote_to_common_type*/, "
                f"{str(ctxop.promote_int_to_float()).lower()}/*promote_int_to_float*/, "
                f"false /*safe_cast*/, "
                f"false /*double_support*/, "
                f"false /*int64_support*/);\n"
            )
            code += "  FALLBACK_IF_UNSUPPORTED_DTYPE{}(compute_type, {}, {}{})\n".format(
                "2" if overload else "",
                opname,
                overload + ", " if overload else "",
                ", ".join(param_vars),
