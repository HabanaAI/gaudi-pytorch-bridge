...
OpGen = namedtuple_with_defaults(
    "OpGen",
    "tree, xtree, rwxtree, func, xfunc, op_frontend, op_backend, cname, sig, rwsig, cppsig, funsig, mapsig, aten_sig, dispatch, default, ctxop, opgroup",
)
...
def get_function_signature(t, orig_sig, namefn, cname):
    emit = StringEmit(orig_sig)
    # Emit full function return type.
    emit_string(typed_child(t, 0, "type"), emit, lambda t: 0)
    fnname = typed_child(t, 1, "fnname").children[0]
    xfname = namefn(fnname.value, "HpuOp_{}".format(cname))
...
def get_hpu_wrapper(fndef, ctx):
    ...
    def gen_fnname(x, cname):
        return "{}::{}".format(cname, x)
    ...
    sig, fname, xfname = get_function_signature(rwxtree, rwsig, gen_fnname, opgroup)
...
def generate_all(fgens):
    ...
    for fgen in fgens:
        ...
        override_fn = "HpuOp_{}::{}".format(fgen.opgroup, fgen.func)
...
def gen_output_file(args, name):
    ...
    if not os.path.exists(args.output_dir):
        ...
        oldfile = os.path.join(os.path.join(args.output_dir, "hpu_op.cpp"))
...
def generate(args):
    ...
    # Find a better way to handle same class in two headers
    frontend_class_headers = {}
    backend_class_headers = {}

    def get_headers(fgens, frontend_class_headers, backend_class_headers):
        headers = set()
        for fgen in fgens:
            ctxop = fgen.ctxop
            headers.add(frontend_class_headers.get(ctxop.get_op_frontend_class(), ""))
            headers.add(backend_class_headers.get(ctxop.get_op_backend_class(), ""))
    ...
    for fgen_file, fgens in fgen_files.items():
        # print(fgen_file, "with", len(fgens), "fgens.")

        op_backend_classes = generate_op_backend_hclasses(
            fgens, backend_class_headers, fgen_file
        )
        op_frontend_classes = generate_op_frontend_hclasses(
            fgens, frontend_class_headers, fgen_file
        )
        fill_params_decls = generate_fill_params_hdecls(fgens)
        outshapes_decls = generate_outshapes_hdecls(fgens)

        optional_header_inclusions = get_headers(
            fgens, frontend_class_headers, backend_class_headers
        )
        ...
        print(
            _CPP_HEADER.format(
                gen=os.path.basename(sys.argv[0]),
                fgen_file=fgen_file,
                optional_header_inclusions=optional_header_inclusions,
            ...
            file=gen_cpp_output_file(args, fgen_file),
        )
