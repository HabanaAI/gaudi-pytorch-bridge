...
OpGen = namedtuple_with_defaults(
    "OpGen",
    "tree, xtree, rwxtree, func, xfunc, lazy_code, hpuop_code, cname, sig, rwsig, cppsig, funsig, mapsig, aten_sig, dispatch, default",
)
...
def lazyop(ctxop, tfetcher, fn, aten_sig, rtype, param_vars, meta_vars, ce_param_vars):
...
def get_hpuop_class_impl(ctxop, fname, cname):
    ...
    if tpc_param:
        ...
        params = []
        for i in tpc_param["params"]:
            if len(i):
                params.append("stack[{}].toScalar().to<{}>()".format(i[0], i[1]))
            else:
                params.append("{}")
        code += ", ".join(params) + "};\n"
...
def generate_code(ctx, tree, rwxtree, fname, aten_sig, sig, rwsig, params):
    ...
    return lazy_code, hpuop_code, hpuop_class
...
def get_hpu_wrapper(fndef, ctx):
    ...
    if requires_registration(fndef) and get_aten_opname(aten_sig) in ctx.op_data:
        lazy_code, hpuop_code, cname = generate_code(
