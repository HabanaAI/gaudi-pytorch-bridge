...
def get_tuple_return(
    rtype, rtype_str, rname, params, param_vars, ref_param, fnopts, out_fn=False
):
...
def generate_return_stmt(
    ...
    if ctype == "std::tuple":
        ...
        retstr = get_return_value(
            rtype, rname, params[ti], param_vars[0], ref_param, fnopts
        )
...
def generate_aten_out(ctx, tree, rwxtree, fname, sig, rwsig, params, fnopts):
    ...
    if fnopts.outfn_template is not None:
        ...
        fcall = create_call("hpu_wrap::{}".format(m.group(1)), non_out_vars)

    tmp_result = "{}_tmp".format(fname)
    code += "  auto {} = {};\n".format(tmp_result, fcall)
    if num_outputs is None:
        code += generate_outfn_result_copy(out_vars[0], tmp_result)
        code += generate_exit_debug_code(tree, fname, out_vars[0], params, param_vars)
        code += "  return {};\n".format(out_vars[0])
    else:
