...
FuncDef = namedtuple_with_defaults(
    "FuncDef", "cpp_sig, aten_sig, compound, has_math_kernel"
)

FuncGen = namedtuple_with_defaults(
    "FuncGen",
    "tree, xtree, rwxtree, func, xfunc, code, sig, rwsig, cppsig, funsig, mapsig, aten_sig, compound, has_math_kernel",
)
...
#include "aten_hpu_type_default.h"
...
namespace habana {{
static HpuFallbackStatistics stat;
...
_FUNCTION_OPTIONS = {
    "slice(Tensor, int64_t, int64_t, int64_t, int64_t) -> Tensor": FuncOpts(
        wparams=["self"]
    ),
}

_RESULT_NAME = "h_result"
...
class TensorFetcher(object):
    ...
    def generate_fetches(self):
        ...
        code += ("  auto {} = habana_lazy::HpuGetFallbackTensorList({});\n").format(
    ...
    def generate_updates(self):
        ...
        if self.writeable:
            ...
            code += "  habana_lazy::HpuUpdateTensors({}, {}, {});\n".format(
...
def get_return_value(rtype, rname, param, var, ref_param, fnopts):
    ...
    if type_is_const(rtype) or type_is_refptr(rtype, "&"):
        ...
        # parameter. In these cases we operated on HPU tensors data (the ATEN one),
        ...
        return "habana_lazy::CreateHpuTensor({}, habana_lazy::GetHpuDevice({}))".format(
...
def get_reference_param(params, fnopts=None):
    ...
    for p in params:
        ...
        if get_optional(fnopts, "ref_param") == pname:
            return p
        if not other and (
            cptype == "TensorOptions" or cptype == "TensorList" or cptype == "Device"
        ):
            other = p
        if cptype != "Tensor":
            continue
...
def get_tuple_return(rtype, rtype_str, rname, params, param_vars, ref_param, fnopts):
...
def generate_return_stmt(
    t, rtype_str, fname, rname, params, param_vars, ref_param, fnopts
):
    assert isinstance(t, lark.tree.Tree)
    rtype = t.children[0]
    ...
    if ctype == "std::tuple":
        retstr = get_tuple_return(
            rtype, rtype_str, rname, params, param_vars, ref_param, fnopts
        )
    elif ctype == "std::vector":
        retstr = (
            "habana_lazy::CreateHpuTensors({}, habana_lazy::GetHpuDevice({}))".format(
                rname, get_optional(fnopts, "device_param", param_name(ref_param))
            )
        )
    elif ctype == "Tensor":
        retstr = get_return_value(
            rtype, rname, params[0], param_vars[0], ref_param, fnopts
        )
...
def get_handling_function(ctx, fname, hpu_ref_param, param_vars):
    function = _HPU_FUNCTIONS.get(fname, None) or ctx.get_function(fname)
    if function:
        code = "{}({})".format(function, ", ".join(param_vars))
    else:
        other_params = list(param_vars)
        other_params.remove(hpu_ref_param)
        code = "{}.{}({})".format(hpu_ref_param, fname, ", ".join(other_params))
    return code
...
def generate_shape_checks(param_vars, shape_check_indices, fname):
    ...
    for i, j in shape_check_indices:
        code += (
            '  TORCH_CHECK({}.sizes() == {}.sizes()) << "Operand shapes must be '
            'identical for {}, mismatch for arguments {} and {}";\n'
        ).format(param_vars[i], param_vars[j], fname, i + 1, j + 1)
    return code
...
def generate_aten_out(ctx, tree, rwxtree, fname, sig, rwsig, params, fnopts):
    ...
    if fnopts.outfn_template is not None:
        ...
        fcall = create_call("hpu_wrap::{}".format(m.group(1)), param_vars[out_count:])

    tmp_result = "{}_tmp".format(fname)
    code += "  auto {} = {};\n".format(tmp_result, fcall)
    if num_outputs is None:
        code += generate_outfn_result_copy(param_vars[0], tmp_result)
        code += generate_exit_debug_code(tree, fname, param_vars[0], params, param_vars)
        code += "  return {};\n".format(param_vars[0])
    else:
        ...
        code += generate_exit_debug_code(
            tree, fname, param_vars[0:num_outputs], params, param_vars
        )
...
def generate_aten_to_hpu(ctx, tree, rwxtree, fname, sig, rwsig, params, fnopts):
    ...
    if set(target_options).issubset(set(param_vars)):
        ...
        code += "  {}{};\n".format(
            result_assign, get_handling_function(ctx, fname, hpu_ref_param, param_vars)
        )
    ...
    if result_assign:
        code += (
            "  static_cast<void>({}); // Avoid warnings in case not "
            "used\n".format(_RESULT_NAME)
        )
    code += generate_exit_debug_code(
        tree, fname, _RESULT_NAME if result_assign else None, params, param_vars
    )
    code += generate_return_stmt(
        tree,
...
def get_hpu_wrapper(fndef, ctx):
    ...
    return FuncGen(
        ...
        has_math_kernel=fndef.has_math_kernel,
    )
...
def create_funcdef(fndef, jdata):
    ...
    return FuncDef(
        ...
        compound=fields.get("compound", "False") == "True",
        has_math_kernel=fields.get("has_math_kernel", "False") == "True",
    )
...
def requires_registration(fgen, overrides):
    requires_lowering = not fgen.compound and not fgen.has_math_kernel
    has_hpu_lowering = get_mapsig_key(fgen.mapsig) in overrides
    return requires_lowering or has_hpu_lowering
...
def gen_h_output_file(args):
    return gen_output_file(args, "aten_hpu_type_default.h")


def gen_cpp_output_file(args):
    return gen_output_file(args, "aten_hpu_type_default.cpp")
...
def generate(args):
    ...
    print(
        "Generated {} wrappers for {}".format(len(fgens), args.typedef), file=sys.stderr
    )
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        ...
        help="The path to the HPU ATEN overrides file",
    )
    arg_parser.add_argument(
        "typedef",
        type=str,
