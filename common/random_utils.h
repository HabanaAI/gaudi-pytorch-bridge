...
#define HABANA_RANDOM_DEF_CHECKPOINT(op_name, inputs)      \
  m.def("hpu::habana_" #op_name "(" inputs ") -> Tensor"); \
  m.def("hpu::habana_" #op_name "_checkpoint(" inputs      \
        ") -> (Tensor, Tensor)");                          \
  m.def("hpu::habana_" #op_name "_checkpoint_backward(" inputs ") -> (Tensor)");

#define HABANA_RANDOM_DEF_CHECKPOINT_2_OUTS(op_name, inputs)         \
  m.def("hpu::habana_" #op_name "(" inputs ") -> (Tensor, Tensor)"); \
  m.def("hpu::habana_" #op_name "_checkpoint(" inputs                \
        ") -> (Tensor, Tensor, Tensor)");                            \
  m.def("hpu::habana_" #op_name "_checkpoint_backward(" inputs       \
        ") -> (Tensor, Tensor)");
