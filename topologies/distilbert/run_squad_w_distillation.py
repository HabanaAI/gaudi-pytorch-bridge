...
def train(args, train_dataset, model, tokenizer, teacher=None, trainMetaData=None):
    """Train the model"""
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            input_keys = ('input_ids', 'attention_mask', 'start_positions')
            ...
            tools.tp_probe_tensors_iteration_start(model, torch.device('hpu'), target, input_dict, trainMetaData.ParamsDump, False)
...
def main():
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, model, tokenizer, teacher=teacher, trainMetaData=trainMetaData)
        if args.local_rank in [-1, 0]:
