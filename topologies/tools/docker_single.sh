...
MOUNT="/data:/root/data -v  /software:/software "
...
CHECK_CONT=`docker ps  | grep ${CONT_NAME} | awk -F' '  '{print $NF}'`
    ...
    if [ "$CHECK_CONT" ==  "$CONT_NAME" ]; then
        ...
        docker run -d -t --privileged=true --env BUILD_NUMBER=$DOCK_NUM  ${PROXY_SETTING} --env MULTI_HLS_IPS=$HOSTNAMES -e DISPLAY= -v  $MOUNT -v /tmp:/tmp  \
