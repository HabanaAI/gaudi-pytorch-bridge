...
synapse_error hcl_communicator::memcpy_within_device(device_ptr source, device_ptr destination, size_t total_bytes,
                                                     std::function<void()> tensor_cleanup_callback) {
  HABANA_ASSERT(my_device_ != nullptr);
  return my_device_->copy_data_within_device(source, destination, total_bytes, std::move(tensor_cleanup_callback));
}

synapse_error hcl_communicator::memcpy_within_device(const device::transfer_manifest& manifest,
                                                     std::function<void()> tensor_cleanup_callback) {
  HABANA_ASSERT(my_device_ != nullptr);
  return my_device_->copy_data_within_device(manifest, std::move(tensor_cleanup_callback));
}

synapse_error hcl_communicator::memcpy_to_device(void* cpu_data, device_ptr destination, size_t total_bytes,
                                                 const event_done_callback& done_cb) {
  HABANA_ASSERT(my_device_ != nullptr);
  return my_device_->copy_data_to_device(cpu_data, destination, total_bytes, done_cb);
}

synapse_error hcl_communicator::memcpy_to_host(device_ptr device_data, void* destination, size_t total_bytes,
                                               const event_done_callback& done_cb) {
  HABANA_ASSERT(my_device_ != nullptr);
  return my_device_->copy_data_to_host(device_data, destination, total_bytes, done_cb);
}

synapse_error hcl_communicator::memcpy_sync_to_host(device_ptr device_data, void* destination, size_t total_bytes) {
  HABANA_ASSERT(my_device_ != nullptr);
  std::mutex mtx;
  std::condition_variable cv;
  std::atomic<bool> done{false};

  synapse_error maybe_error{my_device_->copy_data_to_host(device_data, destination, total_bytes, [&] {
    // Note: It is ok to capture by reference here, as this function by design, must not exit until callback is called.
...
synapse_error_o hcl_communicator::reduce_scatter(device_ptr input_address, device_ptr output_address, size_t elem_cnt,
  ...
  if (!ok(maybe_buffer_ptr)) {
    ...
    LOG_(ERROR) << "Intermediate buffer allocation failed. " << error.error << " Err: " << error.status << ".";
    return error;
  }
