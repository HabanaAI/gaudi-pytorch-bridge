...
std::string get_unique_recipe_name(const std::string& name) {
  ...
  if (env_graph_prefix != nullptr) {
    return absl::StrFormat("%s%s_%s_%d", graph_prefix, env_graph_prefix, name, ++suffix);
  }
...
bool check_and_prepare_graph_dir() {
  if (mkdir(graph_prefix.c_str(), S_IRWXU | S_IRWXG) == 0) {  // NOLINT(hicpp-signed-bitwise)
    return true;
  }

  struct stat info {};
  if (stat(graph_prefix.c_str(), &info) != 0 || !(info.st_mode & S_IFDIR)) {  // NOLINT(hicpp-signed-bitwise))
...
class GraphDirStaticMaker {
 public:
  GraphDirStaticMaker() { check_and_prepare_graph_dir(); }
};
...
graph::graph(device& device, std::string name)
  ...
  static_assert(sizeof(ns_ConstantKernel::Params) == 4, "ns_ConstantKernel::Params has wrong size");
  static_assert(sizeof(ns_Reduction::Params) == 4, "ns_Reduction::Params has wrong size");
  static_assert(sizeof(ns_SpatialReduction::Params) == 44, "ns_SpatialReduction::Params has wrong size");
  static_assert(sizeof(ns_PadKernel::Params) == 44, "ns_PadKernel::Params has wrong size");
  static_assert(sizeof(ns_TileKernel::Params) == 16, "ns_TileKernel::Params has wrong size");
  static_assert(sizeof(ns_BatchNormKernel::Params) == 12, "ns_BatchNormKernel::Params has wrong size");
  static_assert(sizeof(ns_Softmax::Params) == 4, "ns_Softmax::Params has wrong size");
  static_assert(sizeof(ns_SoftmaxCrossEntropy::Params) == 8, "ns_SoftmaxCrossEntropy::Params has wrong size");
}
...
synapse_error_v<graph> graph::create(device& device, std::string name) {
  ...
  auto status = synGraphCreate(syn_graph.graph_handle_.get(), syn_graph.device_.type());
  SYNAPSE_SUCCESS_CHECK_WITH_OP("Graph creation failed.", status, graph::instance_lock_.unlock())
...
synapse_error_o graph::add_node(std::vector<synTensor>&& inputs, std::vector<synTensor>&& outputs, void* const params,
                                const unsigned params_size, std::string&& node_type) {
  if (!in_build_phase_) {
    return synapse_error{"Graph not in build phase.", synStatus::synFail};
  }
  VLOG_(4) << "graph " << name_ << " synGenericNodeCreate(inputs=(" << inputs << "), outputs=(" << outputs
          << "), sizeInputs=" << inputs.size() << ", sizeOutputs=" << outputs.size() << ", userParams=" << params
          << ", params_size=" << params_size << ", guid=" << node_type.c_str() << ", name=\"\");";
  if (current_op_name_) {
    synNodeId nodeId;
    auto status = synNodeCreateWithId(*graph_handle_, inputs.empty() ? nullptr : inputs.data(),
                                      outputs.empty() ? nullptr : outputs.data(), inputs.size(), outputs.size(),
                                      params, params_size, node_type.c_str(), "", &nodeId, nullptr, nullptr);
    ...
    auto status = synNodeCreate(*graph_handle_, inputs.empty() ? nullptr : inputs.data(),
                                outputs.empty() ? nullptr : outputs.data(), inputs.size(), outputs.size(), params,
                                params_size, node_type.c_str(), "", nullptr, nullptr);
    SYNAPSE_SUCCESS_CHECK("Node " + node_type + " add failed.", status)
    graph_is_empty_ = false;
  }
  return {};
...
synapse_error_v<std::shared_ptr<graph::recipe_handle>> graph::compile() {
  ...
  SYNAPSE_SUCCESS_CHECK("Graph compile failed - setting GCFG_DISABLE_DEBUG_INFO=1", status)

  auto name = get_unique_recipe_name(name_);
  status = synGraphCompile(&recipe_handle->syn_recipe_handle_, *graph_handle_, name.c_str(), nullptr);
...
std::string to_string(const std::vector<synLaunchTensorInfo>& patching_info) {
  return absl::StrJoin(patching_info, ",", [](std::string* out, const synLaunchTensorInfo& in) {
    absl::StrAppendFormat(out, "%s:0x%X", in.tensorName, in.pTensorAddress);
  });
}

synapse_error_o graph::create_launch_info(launch_info& handle, const graph::recipe_handle& recipe_handle) {
  synStatus status;

  status = synWorkspaceGetSize(&handle.workspace_buffer_size_, recipe_handle.syn_recipe_handle_);
  SYNAPSE_SUCCESS_CHECK("Getting workspace size failed", status);

  return {};
}

synapse_error_o graph::launch(launch_info& handle, const graph::recipe_handle& recipe_handle,
  ...
  auto table_checker{[&recipe_handle](const synLaunchTensorInfo& info) -> bool {
    if (info.pTensorAddress == 0 || info.tensorName == nullptr) {
      LOG_(ERROR) << recipe_handle.recipe_name_ << " null address:" << (info.pTensorAddress == 0)
  ...
  SYNAPSE_RETURN_IF_ERROR(std::find_if(inputs_and_outputs_info.begin(), inputs_and_outputs_info.end(),
                                       table_checker) == inputs_and_outputs_info.end());
  auto& compute_stream = recipe_handle.device_.get_compute_stream();

  status = synLaunch(compute_stream, inputs_and_outputs_info.data(), inputs_and_outputs_info.size(),
...
synapse_error_v<std::string> graph::name_suffix_from_type(const synDataType type) {
...
graph::recipe_handle::~recipe_handle() {
  if (syn_recipe_handle_ && synRecipeDestroy(syn_recipe_handle_) != synStatus::synSuccess) {
    LOG_(ERROR) << "Failed to destroy recipe!";
  }
}

void graph::collect_dst_synapse_nodes(graph::Op2NodeContainer::mapped_type& dst_synapse_node_ids,
                                      const std::string& dst_node) {
  auto op_to_node_iter = op_to_node_container_.find(dst_node);
  if (op_to_node_iter != op_to_node_container_.end() && !op_to_node_iter->second.empty()) {
    dst_synapse_node_ids.insert(begin(op_to_node_iter->second), end(op_to_node_iter->second));
    return;
  }
...
synStatus graph::set_synapse_control_edges() {
  ...
  for (const auto& nodePair : control_edges_container_) {
    VLOG_(4) << "Starting adding synapse control edges from node " << nodePair.first;
    ...
    if (op_to_node_iter == end(op_to_node_container_) || op_to_node_iter->second.empty() ||
        dst_synapse_node_ids.empty()) {
      // Some ops like NoOp do not have underlying synapse nodes - it is handled in collect_dst_synapse_nodes
      VLOG_(4) << "Ommiting adding synapse control edges from node " << nodePair.first;
      continue;
    }
    const auto& src_synapse_node_ids = op_to_node_iter->second;
    std::vector<synNodeId> src_synapse_node_ids_vector(begin(src_synapse_node_ids), end(src_synapse_node_ids));
    std::vector<synNodeId> dst_synapse_node_ids_vector(begin(dst_synapse_node_ids), end(dst_synapse_node_ids));

    VLOG_(4) << "Adding synapse control edges from node " << nodePair.first;
    status =
        synNodeDependencySet(*graph_handle_, src_synapse_node_ids_vector.data(), dst_synapse_node_ids_vector.data(),
                             src_synapse_node_ids_vector.size(), dst_synapse_node_ids_vector.size());
