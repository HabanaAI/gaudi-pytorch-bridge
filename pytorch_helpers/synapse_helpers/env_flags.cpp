...
#define INST_GETENV_BY_TYPE_WITH_CAST(T, castT)                             \
  template <>                                                               \
  RT<T> getenv_by_type(const char* name, T def_val, T min_val, T max_val) { \
    auto pair = getenv_by_type(                                             \
        name,                                                               \
        static_cast<castT>(def_val),                                        \
        static_cast<castT>(min_val),                                        \
        static_cast<castT>(max_val));                                       \
    return std::make_pair(static_cast<T>(pair.first), pair.second);         \
  }

INST_GETENV_BY_TYPE_WITH_CAST(int, long)
INST_GETENV_BY_TYPE_WITH_CAST(unsigned, unsigned long)
