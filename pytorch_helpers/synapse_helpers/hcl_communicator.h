...
class hcl_communicator {
 public:
  ...
  synapse_error memcpy_within_device(device_ptr source, device_ptr destination, size_t total_bytes,
                                     std::function<void()> tensor_cleanup_callback);

  synapse_error memcpy_within_device(const device::transfer_manifest& manifest,
                                     std::function<void()> tensor_cleanup_callback);

  synapse_error memcpy_to_device(void* cpu_data, device_ptr destination, size_t total_bytes,
                                 const event_done_callback& done_cb);

  synapse_error memcpy_to_host(device_ptr device_data, void* destination, size_t total_bytes,
                               const event_done_callback& done_cb);

  synapse_error memcpy_sync_to_host(device_ptr device_data, void* destination, size_t total_bytes);
 ...
 private:
  ...
  synapse_error_v<owned_device_ptr> alloc_intermediate_buffer(size_t elem_cnt, synDataType elem_type,
