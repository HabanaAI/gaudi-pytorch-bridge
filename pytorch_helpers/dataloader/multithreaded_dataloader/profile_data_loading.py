...
if __name__=='__main__':
    ...
    parser.add_argument('--data-path', default='/software/data/pytorch/imagenet/ILSVRC2012/',
                        help='dataset path')
    parser.add_argument('--dl-worker-type', default='MT', type=lambda x: x.upper(),
                        choices = ["MT", "MP", "ALL"], help='select multithreading or multiprocessing or both')
    parser.add_argument('--num_iterations', default=50, type=int, metavar='N',
    ...
    profile_dataloader_for_resnet(args.data_path, args.workers, args.num_iterations, args.batch_size, args.dl_worker_type)
    # plt.plot(X,Y)
    # plt.show()
