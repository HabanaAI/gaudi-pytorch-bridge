...
bool compare(float* ref, test_t* test, int n) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (prec_margin < 0) {
      std::cerr << "compare failed at index " << i << ", reference value "
                << ref[i] << ", actual value " << test_v << ", absolute error "
                << abs_err << " " << (abs_err < atol ? "ok " : "FAIL")
