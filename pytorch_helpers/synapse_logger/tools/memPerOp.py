...
def list_ops_with_send_size(json_path, json_out_path):
    with open(json_path) as json_file:
        ...
        mem_cp = [obj for obj in timeline if obj['name'] == "synMemCopyAsync"
                  and obj['ts'] > min([x['ts'] for x in send_and_recv])]
        mem_per_obj = [
            {'name': next((x for x in send_and_recv if x['ts'] < memEvent['ts'] < (x['ts'] + x['dur'])
                           and memEvent['pid'] == x['pid']
                           and memEvent['tid'] == x['tid']), None)['args']['name'],
