...
HCLStatus (*HCL_IAllreduce)(
    ...
    HCL_Comm communicator,
    const uint32_t flags);
HCLStatus (*HCL_Get_Intermediate_Buffer_size)(
    uint64_t* intermediateSize,
...
void LoadSymbols(void* lib_handle) {
  ...
  CHECK_NULL(
      HCL_Destroy = (decltype(HCL_Destroy))dlsym(lib_handle, "HCL_Destroy"));
  CHECK_NULL(
      HCL_Comm_Size =
          (decltype(HCL_Comm_Size))dlsym(lib_handle, "HCL_Comm_Size"));
  CHECK_NULL(
      HCL_Comm_Rank =
          (decltype(HCL_Comm_Rank))dlsym(lib_handle, "HCL_Comm_Rank"));
  CHECK_NULL(
      HCL_Comm_Ranks =
          (decltype(HCL_Comm_Ranks))dlsym(lib_handle, "HCL_Comm_Ranks"));
  CHECK_NULL(HCL_Wait = (decltype(HCL_Wait))dlsym(lib_handle, "HCL_Wait"));
  CHECK_NULL(
      HCL_Allreduce =
          (decltype(HCL_Allreduce))dlsym(lib_handle, "HCL_Allreduce"));
  CHECK_NULL(
      HCL_IAllreduce =
  ...
  CHECK_NULL(
      HCL_Get_Intermediate_Buffer_size =
          (decltype(HCL_Get_Intermediate_Buffer_size))dlsym(
              lib_handle, "HCL_Get_Intermediate_Buffer_size"));
  ...
  CHECK_NULL(
      HCL_Reduce = (decltype(HCL_Reduce))dlsym(lib_handle, "HCL_Reduce"));
  CHECK_NULL(
      HCL_IReduce = (decltype(HCL_IReduce))dlsym(lib_handle, "HCL_IReduce"));
  CHECK_NULL(
      HCL_Reduce_Scatter = (decltype(HCL_Reduce_Scatter))dlsym(
          lib_handle, "HCL_Reduce_Scatter"));
  CHECK_NULL(
      HCL_IReduce_Scatter = (decltype(HCL_IReduce_Scatter))dlsym(
          lib_handle, "HCL_IReduce_Scatter"));
  CHECK_NULL(
      HCL_AllGather =
          (decltype(HCL_AllGather))dlsym(lib_handle, "HCL_AllGather"));
  CHECK_NULL(
      HCL_IAllGather =
          (decltype(HCL_IAllGather))dlsym(lib_handle, "HCL_IAllGather"));
  CHECK_NULL(
      HCL_NetworkFlush =
...
std::ostream& operator<<(std::ostream& os, const HCL_Request request) {
  os << "\"{" << request.event << ", " << request.index << ", "
     << request.pIndex << "}\"";
  return os;
}
...
HCLStatus HCL_Allreduce(
  ...
  API_LOG_CALL(
      ...
      ARG_Q(communicator),
      ARG_X(flags));
  HCLStatus status = lib_hcl::HCL_Allreduce(
      streamHandle,
      sendBuffAddr,
      receiveBuffAddr,
      count,
...
HCLStatus HCL_IAllreduce(
  ...
  API_LOG_CALL(
      ...
      ARG_Q(communicator),
      ARG_X(flags));
  HCLStatus status = lib_hcl::HCL_IAllreduce(
      phRequest,
      sendBuffAddr,
      receiveBuffAddr,
      count,
...
HCLStatus HCL_Get_Intermediate_Buffer_size(
    ...
    const HCL_Comm communicator) {
  API_LOG_CALL(
      ARG(intermediateSize),
      ARG_X(collectiveOp),
      ARG_X(count),
      ARG_X(dataType),
      ARG_Q(communicator));
  HCLStatus status = lib_hcl::HCL_Get_Intermediate_Buffer_size(
      intermediateSize, collectiveOp, count, dataType, communicator);
...
HCLStatus HCL_Bcast(
  ...
  API_LOG_CALL(
      ...
      ARG_Q(communicator),
      ARG_X(flags));
  HCLStatus status = lib_hcl::HCL_Bcast(
      streamHandle,
      sendBuffAddr,
      receiveBuffAddr,
      count,
...
HCLStatus HCL_IBcast(
  ...
  API_LOG_CALL(
      ...
      ARG_Q(communicator),
      ARG_X(flags));
  HCLStatus status = lib_hcl::HCL_IBcast(
      phRequest, Address, count, dataType, root, communicator, flags);
  API_LOG_RESULT(S_ARG_X(phRequest));
  return status;
}
...
HCLStatus HCL_Reduce(
  ...
  API_LOG_CALL(
      ...
      ARG_Q(communicator),
      ARG_X(flags));
  HCLStatus status = lib_hcl::HCL_Reduce(
      streamHandle,
      sendBuffAddr,
      receiveBuffAddr,
      count,
...
HCLStatus HCL_IReduce(
  ...
  API_LOG_CALL(
      ...
      ARG_Q(communicator),
      ARG_X(flags));
  HCLStatus status = lib_hcl::HCL_IReduce(
      phRequest,
      sendBuffAddr,
      receiveBuffAddr,
      count,
...
HCLStatus HCL_Reduce_Scatter(
  ...
  API_LOG_CALL(
      ...
      ARG_Q(communicator),
      ARG_X(flags));
  HCLStatus status = lib_hcl::HCL_Reduce_Scatter(
      streamHandle,
      sendBuffAddr,
      receiveBuffAddr,
      count,
...
HCLStatus HCL_IReduce_Scatter(
  ...
  API_LOG_CALL(
      ...
      ARG_Q(communicator),
      ARG_X(flags));
  HCLStatus status = lib_hcl::HCL_IReduce_Scatter(
      phRequest,
      sendBufAddr,
      receiveBuffAddr,
      count,
...
HCLStatus HCL_AllGather(
  ...
  API_LOG_CALL(
      ...
      ARG_Q(communicator),
      ARG_X(flags));
  HCLStatus status = lib_hcl::HCL_AllGather(
      streamHandle,
      sendBufAddr,
      receiveBuffAddr,
      count,
...
HCLStatus HCL_IAllGather(
  ...
  API_LOG_CALL(
      ...
      ARG_Q(communicator),
      ARG_X(flags));
  HCLStatus status = lib_hcl::HCL_IAllGather(
      phRequest,
      sendBufAddr,
      receiveBuffAddr,
      count,
