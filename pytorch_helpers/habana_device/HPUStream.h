...
#else
  c10::StreamData3 pack() const noexcept {
    return stream_.pack3();
  }
#endif

// Unpack a HPUStream from the uint64_t representation generated by pack().
#if IS_PYTORCH_OLDER_THAN(1, 14)
  static HPUStream unpack(uint64_t bits) {
    return HPUStream(Stream::unpack(bits));
  }
#else
  static HPUStream unpack(const c10::StreamData3& data) {
    return HPUStream(
        Stream::unpack3(data.stream_id, data.device_index, data.device_type));
  }
#endif
