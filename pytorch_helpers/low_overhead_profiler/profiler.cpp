...
ProfilerEngine::ProfilerEngine()
  ...
  if ((disable_string == nullptr) ||
      (static_cast<uint32_t>(std::stoi(disable_string)) == 0U)) {
    ...
    auto overhead = (fragmented_test_result - consolidated_test_result) /
        999; // NOLINT(readability-magic-numbers)
...
uint64_t calculate_adaptive_cutoff(
    ...
    uint64_t max_time,
    double percentile = 0.9) { // NOLINT(readability-magic-numbers)
  if (event_times.empty()) {
    return max_time;
  }
  std::sort(event_times.begin(), event_times.end());
  auto cutoff_index = static_cast<uint64_t>(
...
void create_buckets(
  ...
  exponential_base = (exponential_range > 0)
      ? (1ULL << NUM_EXPONENTIAL_BUCKETS) // 2^NUM_EXPONENTIAL_BUCKETS
      : 1;
...
void print_histogram(
  ...
  std::ignore = fprintf(
      metrics_file, "        ------------------------------------------\n");
  std::ignore = fprintf(metrics_file, "        Time Range (ns)\tFrequency\n");
  std::ignore = fprintf(
      metrics_file, "        ------------------------------------------\n");

  for (size_t i = 0; i < NUM_EQUIDISTANT_BUCKETS;
  ...
  for (size_t i = 0; i < NUM_EXPONENTIAL_BUCKETS; ++i) {
    uint64_t bucket_min = prev_max + 1;
    uint64_t bucket_max = (i == NUM_EXPONENTIAL_BUCKETS - 1)
        ? max_time
        : (bucket_min +
           ((static_cast<uint64_t>(1) << i) * exponential_range /
            exponential_base) -
           1);
    std::ignore = fprintf(
        metrics_file,
...
void print_device_queue_histogram(
  ...
  std::ignore = fprintf(metrics_file, "   Device Queue Depth Percentage");
  std::ignore =
      fprintf(metrics_file, "   ------------------------------------------\n");
...
bool all_stages_empty(
    const std::unordered_map<size_t, std::unordered_map<std::string, uint64_t>>&
        stage_counter) {
  return std::all_of(
...
void ProfilerEngine::flush() {
  ...
  this->enabled = false;
  std::unordered_map<size_t, std::unordered_map<std::string, uint64_t>>
      stage_total_time;
  std::unordered_map<size_t, std::unordered_map<std::string, uint64_t>>
      stage_counter;
  std::unordered_map<size_t, std::unordered_map<std::string, uint64_t>>
  ...
  std::array<uint64_t, NUM_OF_PIPELINE_STAGES> current_index = {0, 0, 0, 0, 0};
  ...
  FilePtr events_file = FilePtr(fopen(event_file_path.c_str(), "w"), fclose);
  std::ignore = fprintf(
      events_file.get(), "{\"displayTimeUnit\": \"ns\", \"traceEvents\": [\n");

  FilePtr metrics_file = FilePtr(fopen(metric_file_path.c_str(), "w"), fclose);

  FilePtr timing_data_file =
      FilePtr(fopen(timing_data_file_path.c_str(), "w"), fclose);
  std::ignore = fprintf(timing_data_file.get(), "{\n");
  std::ignore = fprintf(timing_data_file.get(), "  \"metadata\": {\n");
  std::ignore = fprintf(timing_data_file.get(), "    \"pid\": %u\n", getpid());
  std::ignore = fprintf(timing_data_file.get(), "  },\n");
  // Find first event, timewise.
  uint64_t tsc_base = std::numeric_limits<uint64_t>::max();
  for (size_t pipeline_stage = 0; pipeline_stage < NUM_OF_PIPELINE_STAGES - 1;
       pipeline_stage++) {
  ...
  for (size_t pipeline_stage = 0; pipeline_stage < NUM_OF_PIPELINE_STAGES - 1;
       pipeline_stage++) {
    this->events_counter[pipeline_stage].load(std::memory_order_acquire);
    for (size_t i = 0; i < current_index[pipeline_stage]; ++i) {
  ...
  std::unordered_map<
      ...
      std::unordered_map<std::string, std::vector<uint64_t>>>
      queue_lengths;
  std::unordered_map<size_t, std::vector<uint64_t>> device_queue_lengths;
  std::unordered_map<
      size_t,
      std::unordered_map<std::string, std::pair<uint64_t, uint32_t>>>
  ...
  for (size_t pipeline_stage = 0; pipeline_stage < NUM_OF_PIPELINE_STAGES - 1;
    ...
    if (current_index[pipeline_stage] > 0) {
      for (size_t i = 0; i < current_index[pipeline_stage]; ++i) {
        ...
        if (event.is_begin) {
          ...
          if ((pipeline_stage ==
              ...
              (event.jit_cache_hit_count > jit_cache_hit_count_threshold)) {
            if (!event_stack[pipeline_stage].empty()) {
              ...
              if (begin_event.name == event.name) {
                ...
                if (pipeline_stage ==
                    static_cast<size_t>(
                        LOP::PipelineStageID::
                            PIPELINE_STAGE_EXECUTE_ID)) { // Collect samples
                  // only at execute
                  // stage
      ...
      if (pipeline_stage ==
          static_cast<int>(LOP::PipelineStageID::PIPELINE_STAGE_EXECUTE_ID)) {
        if (total_events_execute_stage > 0) {
          ...
          for (uint64_t saved_device_queue_length :
            ...
            const int64_t device_queue_length_diff =
                static_cast<int64_t>(saved_device_queue_length) -
                static_cast<int64_t>(mean_device_queue_length);
            device_queue_length_variance +=
                device_queue_length_diff * device_queue_length_diff;
          }

          device_queue_length_variance = device_queue_length_variance /
              (static_cast<int64_t>(total_events_execute_stage) - 1);
  ...
  if (all_stages_empty(stage_counter)) {
    ...
    std::ignore = remove(event_file_path.c_str());
    std::ignore = remove(metric_file_path.c_str());
    std::ignore = remove(timing_data_file_path.c_str());
  } else {
    ...
    for (size_t pipeline_stage = 0; pipeline_stage < NUM_OF_PIPELINE_STAGES - 1;
      ...
      for (const auto& [event_name, ops_map] :
        ...
        for (const auto& [op_name, stats] : ops_map) {
          aggregated_ops.emplace_back(
              op_name, stats.first, static_cast<uint32_t>(stats.second));
        }
    ...
    timing_data_file.reset(fopen(
        timing_data_file_path.c_str(),
        "a")); // NOLINT(cppcoreguidelines-owning-memory)
    for (size_t pipeline_stage = 0; pipeline_stage < NUM_OF_PIPELINE_STAGES - 1;
         pipeline_stage++) {
      if (current_index[pipeline_stage] == 0) {
        continue;
      ...
      if (pipeline_stage ==
        ...
        stage_name = "lowering";
          ...
          pipeline_stage ==
          static_cast<int>(LOP::PipelineStageID::PIPELINE_STAGE_COMPILE_ID)) {
        stage_name = "compile";
      } else {
        stage_name = "execute";
      }
      std::ignore =
          fprintf(timing_data_file.get(), "  \"%s\": {\n", stage_name);
      // 1. sort_by_avg_time
      std::ignore =
          fprintf(timing_data_file.get(), "    \"sort_by_avg_time\": [\n");
      ...
      for (size_t i = 0; i < avg_time_ops.size(); ++i) {
        ...
        std::ignore = fprintf(
            timing_data_file.get(),
            "      {\"op_name\": \"%s\", \"avg_time_ns\": %lu, \"count\": %u, \"total_time\": %lu}",
            op.first.c_str(),
            op.second.first / op.second.second,
            op.second.second,
            op.second.first);
        if (i != avg_time_ops.size() - 1) {
          std::ignore = fprintf(timing_data_file.get(), ",");
        }
        std::ignore = fprintf(timing_data_file.get(), "\n");
      }
      ...
      std::ignore =
          fprintf(timing_data_file.get(), "    \"sort_by_single_time\": [\n");
      ...
      std::partial_sort(
          single_events.begin(),
          single_events.begin() +
              static_cast<decltype(single_events.begin())::difference_type>(
                  report_count),
          single_events.end(),
          [](const auto& a, const auto& b) {
      ...
      for (size_t i = 0; i < report_count; ++i) {
        ...
        std::ignore = fprintf(
            timing_data_file.get(),
            "      {\"op_name\": \"%s\", \"time_ns\": %lu, \"tid\": %u, \"start_ns\": %lu}",
            std::get<0>(event).c_str(),
            std::get<1>(event),
            static_cast<unsigned int>(std::get<3>(event)),
            std::get<2>(event));
        if (i != report_count - 1) {
          std::ignore = fprintf(timing_data_file.get(), ",");
        }
      ...
      std::ignore =
          fprintf(timing_data_file.get(), "    \"sort_by_total_time\": [\n");
      for (size_t i = 0; i < avg_time_ops.size(); ++i) {
        const auto& op = avg_time_ops[i];
        std::ignore = fprintf(
            timing_data_file.get(),
            "      {\"op_name\": \"%s\", \"total_time_ns\": %lu, \"count\": %u}",
            op.first.c_str(),
            op.second.first,
            op.second.second);
        if (i != avg_time_ops.size() - 1) {
      ...
      // 4. all_ops_histogram
      std::ignore =
          fprintf(timing_data_file.get(), "    \"all_ops_histogram\": {\n");
      bool first_op = true;
      for (const auto& [op_name, times] : stage_op_histograms[pipeline_stage]) {
        ...
        first_op = false;
        std::ignore =
            fprintf(timing_data_file.get(), "      \"%s\": [", op_name.c_str());
        // sampling at most NUM_TIMING_DATA_CNTS, which has a default value of
        // 100.
        size_t sample_count =
            std::min<size_t>(NUM_TIMING_DATA_CNTS, times.size());
        ...
        for (size_t i = 0; i < times.size(); i += step) {
          ...
          std::ignore = fprintf(timing_data_file.get(), "%lu", times[i]);
          if (i + step >= times.size()) {
            break;
          }
        }
        std::ignore = fprintf(timing_data_file.get(), "]");
    ...
    for (size_t i = 0; i < NUM_OF_PIPELINE_STAGES - 1; ++i) {
    ...
    for (size_t pipeline_stage = 0; pipeline_stage < NUM_OF_PIPELINE_STAGES - 1;
      ...
      if (!stage_counter[pipeline_stage].empty()) {
        if (pipeline_stage ==
            static_cast<int>(LOP::PipelineStageID::PIPELINE_STAGE_MAIN_ID)) {
          stage_name = "Main";
          std::ignore = fprintf(metrics_file.get(), "\n MAIN STAGE \n");
        } else if (
            pipeline_stage ==
            static_cast<int>(
        ...
        for (const auto& [event_name, count] : stage_counter[pipeline_stage]) {
          if (count > 0) {
            ...
            if (pipeline_stage !=
              ...
              std::ignore = fprintf(
                  metrics_file.get(),
                  "     standard deviation queue length = %ld \n",
                  stage_queue_length_std[pipeline_stage][event_name]);
            }
            std::ignore = fprintf(
                metrics_file.get(), "     Top 5 ops with highest time: \n");
            for (size_t i = 0;
                 i <
                 std::min<size_t>(
                     NUM_TOP_OPS,
    ...
    if (GET_ENV_FLAG_NEW(PT_HPU_ENABLE_LOP_TRACES_COLLECTION) ||
      ...
      auto pid = getpid();
      for (size_t pipeline_stage = 0;
           pipeline_stage < NUM_OF_PIPELINE_STAGES - 1;
           pipeline_stage++) {
        for (size_t i = 0; i < current_index[pipeline_stage]; ++i) {
          auto& event = this->events_table[pipeline_stage][i];
          auto tsc_diff = event.timestamp - tsc_base;
      ...
      std::ignore = fprintf(events_file.get(), "{}]}");
    }
  }
  this->flushed = true;
...
void emit_event_fast(
  ...
  std::lock_guard<std::mutex> lock(
      profiler_engine_instance
          .events_mutex[static_cast<size_t>(pipe_stage_id)]);
  uint64_t current_index =
      profiler_engine_instance
          .events_counter[static_cast<size_t>(pipe_stage_id)]
  ...
  if (current_index < GET_ENV_FLAG_NEW(PT_HPU_EVENT_TABLE_SIZE)) {
    ...
    Event& new_event =
        profiler_engine_instance
            .events_table[static_cast<size_t>(pipe_stage_id)][current_index];
    new_event.timestamp = tsc;
    new_event.name = name;
    new_event.op_name = op_name;
    new_event.thread_id = static_cast<uint64_t>(pthread_self());
    new_event.is_begin = is_begin;
    new_event.pipeline_stage_id = static_cast<int32_t>(pipe_stage_id);
    ...
    profiler_engine_instance.events_counter[static_cast<size_t>(pipe_stage_id)]
