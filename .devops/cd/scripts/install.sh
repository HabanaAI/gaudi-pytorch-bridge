...
PYTHON_MPI_VERSION="${MPI_VERSION:-3.1.4}"
# define constants
...
python${MIN_PYTHON_VER} -m pip install $(grep -ivE "#|lightning" ./requirements-pytorch.txt | grep .) --no-warn-script-location --disable-pip-version-check ${PIP_PYTHON_OPTIONS}
...
if [ -z $HABANALABS_LOCAL_DIR ]; then
    ...
    python${MIN_PYTHON_VER} -m pip install ${HABANALABS_LOCAL_DIR}/habana_lightning_plugins-${HABANA_RELEASE_VERSION}.${HABANA_RELEASE_ID}*.whl --disable-pip-version-check
fi
