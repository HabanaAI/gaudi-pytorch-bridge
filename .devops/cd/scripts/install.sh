...
if [[ -n $TORCH_TYPE && $TORCH_TYPE == "upstream" ]]; then
  rm -f torch-*.whl
  python${MIN_PYTHON_VER} -m pip install torch==${TORCH_VERSION} torchvision==${TORCHVISION_VERSION} torchaudio==${TORCHAUDIO_VERSION} torchdata==${TORCHDATA_VERSION} --index-url https://download.pytorch.org/whl/test/cpu --extra-index-url https://download.pytorch.org/whl/cpu
  ...
  python${MIN_PYTHON_VER} -m pip install torchvision==${TORCHVISION_VERSION} torchaudio==${TORCHAUDIO_VERSION} torchdata==${TORCHDATA_VERSION} --index-url https://download.pytorch.org/whl/test/cpu --extra-index-url https://download.pytorch.org/whl/cpu --dry-run --report pip_report
