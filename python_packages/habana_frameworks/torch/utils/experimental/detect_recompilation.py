...
class Table:
    ...
    def _fattest_per_col(self):
        return [max([len(str(row[col_idx])) for row in self.rows])
                for col_idx in range(len(self.field_names))]

    def _budget_per_col(self, ):
    ...
    def __repr__(self):
        ...
        st = ''
        def row_boundary(
            ch): return '\033[96m' + ch * (sum(budget_col) + num_cols + 1) + '\033[0m' + '\n'
        for rowidx, row in enumerate(self.rows):
            if rowidx == 0:
                st += row_boundary('=')
            assert len(budget_col) == len(row)
            # splitting a row across multiple rows
            max_rows_needed = max([int(math.ceil(len(str(item)) / char_budget))
                                   for item, char_budget in zip(row, budget_col)])
            start_col = ('', '\033[91m')[rowidx == 0]
            end_col = ('', '\033[1m')[rowidx == 0]
            for k in range(max_rows_needed):
                for col_idx, item in enumerate(row):
                    orig_str = str(item)
                    slice = orig_str[k*budget_col[col_idx]
                        : (k+1)*budget_col[col_idx]]
                    st += ('', '\033[96m|\033[0m')[col_idx == 0] + start_col + slice + end_col + \
                        ' '*(budget_col[col_idx] -
                             len(slice)) + '\033[96m|\033[0m'
                st += '\n'
...
def _prettyprint(recompiling_modules, recompiling_modules_count, csv_out):
    ...
    _make_table(["Step", "Recompiling modules", "New in", "New out", "Class",
                "Location", "Comment"], sorted_by_step, 120, csv_out+'_1.csv')
    _make_table(["Module name", "Recompile count"], [(k, [[v]]) for k, v in sorted(
        recompiling_modules_count.items(), key=lambda kv: -kv[1])], 80, csv_out+'_2.csv')
...
def _process_tree(root, output, potential_dyn_modules):
    ...
    for k in root.children:
        dyn_from_children, curr_immediate_child_recompiled = _process_tree(
            k, [], potential_dyn_modules)
...
def _parse(lines):
    recompile_detector_lines = [
        ln for ln in lines if 'DETECT_RECOMPILE_AUTO' in ln]
    recompiling_modules = {}
    ...
    for ln in recompile_detector_lines:
        if '[DETECT_RECOMPILE_AUTO] Registering hooks for' in ln:
            ...
            lhs, rhs = splt[1].split('of type')[1].split(
                'probably defined here')
            classnm = lhs.strip().split('class')[
                1].strip('>').strip().strip("'")
            ...
            if mdlname in module_files:
                ...
                assert (mdlname, classnm, modulenm, filenm) != (
                    mdlname, classnm1, modulenm1, filenm1)
        ...
        if 'Recompilation' in ln:
            ...
            tmp_list += [(module_name, new_inp, new_out,
                          classnm, filenm, comment)]
            recompiling_modules_count[module_name] = recompiling_modules_count.get(
                module_name, 0) + 1
        if step_done:
            step = step_from_ln
            step_done = False
    ...
    treeinfo = []
    treeinfo, _ = _process_tree(
        created_nodes[top_module_name], treeinfo, potential_dyn_modules)
    for step_idx in recompiling_modules:
        for idx, (module_name, new_inp, new_out, classnm, filenm, comment) in enumerate(recompiling_modules[step_idx]):
            if 'Already processed input shape still recompiled' in comment and module_name not in treeinfo:
                ...
                recompiling_modules[step_idx][idx] = (
                    module_name, new_inp, new_out, classnm, filenm, comment)
...
def _wrap_fn(old_fn, tag1, write_to, level=0, waittime=1):
    ...
    def forward(self, *args, **kwargs):
        assert not torch.distributed.is_initialized(), \
            'Expected 1x run, but torch being used in distributed fashion'
        ...
        debug_str = f'{"  "*level}{TAG}[{tag1}]{step_string} {new_inp_string}{new_out_string}num_graphs:{num_graphs} ' + (
...
def detect_recompilation_auto_model(model, mdlname="Net", waittime=1, csv_out='out'):
    ...
    def helper(model, write_to, mdlname="Net", level=0, waittime=1):
        ...
        if hasattr(model, DYNSHAPE_FIELD):
            assert hash(
                model) in edited_modules, f'The model already has field {DYNSHAPE_FIELD}'
        ...
        registration = f'{"  "*level}{TAG} Registering hooks for ' + mdlname + ' of type ' + \
            str(model.__class__) + ' probably defined here ' + \
        ...
        model.forward = MethodType(_wrap_fn(
            model.forward, mdlname, level=level, waittime=waittime, write_to=write_to), model)
        for name, layer in model.named_children():
            layer = helper(layer, write_to, mdlname+'/' +
                           name, level+1, waittime=waittime)
        if level == 0:
            assert not hasattr(
                model, 'raw_logs'), f'The model already has field raw_logs'
            model.raw_logs = MethodType(lambda self: write_to, model)
            assert not hasattr(
                model, 'analyse_dynamicity'), f'The model already has field analyse_dynamicity'
            model.analyse_dynamicity = MethodType(
                _get_analyser(csv_out), model)
        return model
...
def const_shape_dataloader(dl, maxlen):
    ...
    '''
    def helper(lst): return DataLoader(lst, batch_size=None, collate_fn=None)
    const_shape_dt = {}
    for dt in dl:
        shp_key = get_shape(dt)
