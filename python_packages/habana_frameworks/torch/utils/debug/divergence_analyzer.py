    ...
    def compare_databases(self, db_file1, db_file2):
        ...
        for table_name in tables1[2:]:
            ...
            if len(rows_table1) != len(rows_table2):
                self.log(
                    "[WARNING]", table_name[0] + " has different tensor numbers in static and dynamic not comparing"
                )
            else:
    ...
    def dump_csv(self):
        ...
        for graph_name, tensors in self.mismatch_map.items():
            for tensor in tensors:
                ...
                cmd_static = (
                    f"{json_tests_bin} db_parser -d {path_static} -g '{graph_name}' -t '{tensor}' -o {output_static}"
                )
                cmd_dynamic = (
                    f"{json_tests_bin} db_parser -d {path_dynamic} -g '{graph_name}' -t '{tensor}' -o {output_dynamic}"
                )
...
def get_args():
    ...
    parser.add_argument(
        "--do_split", type=int, default=1, help="If set to 1, split the synapse graphs/tensors to multiple files"
    )
    parser.add_argument(
        "--enable_parallel",
        type=int,
        default=1,
        help="If set to 0, forces single card training even when multiple devices are available",
    )
    parser.add_argument(
        "--to_csv", type=int, default=0, help="If set to 1, dumps the output to a CSV file in addition to the TXT file"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="If set to 1, program exits after encountering the very first graphs mismatch",
    )
