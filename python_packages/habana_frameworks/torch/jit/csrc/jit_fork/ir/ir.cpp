...
void Graph::lint() const {
  ...
  void Value::replaceAllUsesDominatedByNodeWith(
      const Node* node, Value* newValue) {
    std::for_each(uses_.begin(), uses_.end(), [&node, newValue](Use& u) {
  ...
  static size_t findArgument(
      const FunctionSchema& the_schema, const std::string& unqualName) {
  ...
  bool Node::matches(
      const char* signature_literal, at::ArrayRef<Symbol> const_inputs) const {
    if (!matches(
  ...
  Node* Graph::create(
      NodeKind kind, ArrayRef<Value*> inputs, size_t num_outputs) {
  ...
  Node* Graph::createTuple(
      at::ArrayRef<Value*> values, TupleTypePtr tuple_type) {
    if (!tuple_type) {
  ...
  Node* Graph::createTupleIndex(
      Value * tup, Value * idx, const TypePtr& output_type) {
  ...
  Node* Graph::createList(
      const TypePtr& contained_type, at::ArrayRef<Value*> values) {
  ...
  Value* Graph::packValues(
      at::ArrayRef<Value*> values, c10::OptNameList field_names) {
    if (values.size() == 1) {
      return values[0];
    }
  ...
  Node* Graph::createSetAttr(
      Value * obj, const std::string& field, Value* newValue) {
  ...
  Value* Graph::insertFunctionCall(
      torch::jit::Function * callee, const MatchedSchema& matched) {
  ...
  Value* Graph::insertMethodCall(
      std::string method_name, const MatchedSchema& matched) {
  ...
  std::vector<Value*> unpackOutputs(const std::vector<Value*>& outputs) {
    std::vector<Value*> new_outputs;
    if (outputs.size() != 1 ||
        outputs.at(0)->type()->kind() != TupleType::Kind) {
      return outputs;
    }
  ...
  std::vector<Node*> findAllNodes(
      at::ArrayRef<Block*> array, Symbol kind, bool recurse) {
    std::vector<Node*> ret;
  ...
  std::vector<Value*> insertGraph(
      Graph & g, Graph & callee, ArrayRef<Value*> inputs) {
    std::unordered_map<Value*, Value*> value_map;
  ...
  std::vector<std::shared_ptr<torch::jit::Operator>> OperatorSet::getOps()
      const {
