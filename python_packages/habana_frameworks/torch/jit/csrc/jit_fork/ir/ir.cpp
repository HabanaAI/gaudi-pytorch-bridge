  ...
  auto checkValue = [&](const Value* v) {
    if (TensorTypePtr type = v->type()->cast<TensorType>()) {
      if (type->device() && !has_device) {
        ...
        // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
        // GAUDI_JIT_ASSERT(device == type->device());
      }
    }
  };
...
void Node::lint() const {
   ...
   * != input->uses_.end());
   *         GAUDI_JIT_ASSERT(graph_->all_nodes.count(this) == 1);
   *         i++;
   * }
   * }
   */
   ...
   *     GAUDI_JIT_ASSERT(use.user->inputs_[use.offset] == o);
   *   }
   * }
   */
  ...
  switch (kind()) {
    case prim::Constant:
      // GAUDI_JIT_ASSERT(inputs_.empty());
      break;
    case prim::Return:
      // Return uses is zero
      // GAUDI_JIT_ASSERT(outputs().empty());
      break;
    case prim::Param:
...
void Graph::lint() const {
  ...
  struct LintScope {
    ...
    void insert(const Value* v) {
      // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
      // GAUDI_JIT_ASSERT(!contains(v));
      values.insert(v);
    }
    void insert(const Node* n) {
      // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
      // GAUDI_JIT_ASSERT(!contains(n));
      nodes.insert(n);
    }
  ...
  struct LintImpl {
    ...
    void check_value(const Value* v) {
       ...
       * GAUDI_JIT_ASSERT(b2.second); // insertion took place
       * GAUDI_JIT_ASSERT(v->unique() < g.next_unique_);
       */

      for (auto use : v->uses()) {
        // GAUDI_JIT_ASSERT(!scope->contains(use.user));
        // GAUDI_JIT_ASSERT(g.all_nodes.count(use.user) == 1);
    ...
    void check_node(const Node* n) {
      for (auto input : n->inputs_) {
        if (!scope->contains(input)) {
          // GAUDI_JIT_ASSERT(0, input->unique(), " not in scope");
        }
      }
      /* GAUDI_JIT_ASSERT(
      ...
      for (auto o : n->outputs()) {
        /*     GAUDI_JIT_ASSERT(o->node() == n);
         *     GAUDI_JIT_ASSERT(i++ == o->offset_);*/
        check_value(o);
      }
      n->lint();
    }
    ...
    void check_block(const Block* b) {
      ...
      // GAUDI_JIT_ASSERT(b->param_node()->isBefore(*b->nodes().begin()));
      auto curNode = *b->nodes().begin();
      while (curNode != b->return_node()) {
        // GAUDI_JIT_ASSERT(curNode->isBefore(curNode->next()));
        curNode = curNode->next();
      }

      for (auto input : b->inputs()) {
        check_value(input);
        // GAUDI_JIT_ASSERT(input->node()->kind_ == prim::Param);
      }

      for (auto n : b->nodes()) {
        // GAUDI_JIT_ASSERT(n->kind_ != prim::Param);
        // GAUDI_JIT_ASSERT(n->kind_ != prim::Return);
        check_node(n);
      }

      // GAUDI_JIT_ASSERT(b->output_->kind() == prim::Return);
      ...
      // GAUDI_JIT_ASSERT(std::includes(ALL_OF(all_nodes_set),
      // ALL_OF(nodes_set))); GAUDI_JIT_ASSERT(
    ...
    void check_graph() {
       ...
       * GAUDI_JIT_ASSERT(kv.second == -1);
       *}
       * GAUDI_JIT_ASSERT(std::includes(ALL_OF(sum_set),
       *     ALL_OF(all_nodes_set)));
       */
    }
  };
...
void Block::reIndexTopology() {
  ...
  for (auto node : nodes()) {
    // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
    // GAUDI_JIT_ASSERT(curPos <= (kUpperBound - kAppendInterval));
...
void Value::replaceFirstUseWith(Value* newValue) {
  // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
  // GAUDI_JIT_ASSERT(owningGraph() == newValue->owningGraph());
  auto u = uses()[0];
...
bool Node::hasSideEffects() const {
  ...
  if (!op) {
    ...
    //     " doesn't have one either. We don't know if this op has side
  ...
  if (kind_.is_prim() || kind_.is_aten() || kind_.is_cuda()) {
    ...
    //     "aten:: and prim:: ops should have
    //     c10::AliasAnalysisKind::INTERNAL_SPECIAL_CASE"
    //     ", c10::AliasAnalysisKind::FROM_SCHEMA or
    //     c10::AliasAnalysisKind::CONSERVATIVE but ", kind_.toDisplayString(),
    //     " has ",
  ...
  // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
  // GAUDI_JIT_ASSERT(false, "Unhandled c10::AliasAnalysisKind case");
...
void Node::assignTopoPosition() {
  ...
  if (is_last) {
    ...
    // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
    // GAUDI_JIT_ASSERT(prevPos < topo_position_ && topo_position_ < nextPos);
  }
}
...
void Node::eraseOutput(size_t i) {
  // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
  // GAUDI_JIT_ASSERT(i < outputs_.size());
  // GAUDI_JIT_ASSERT(outputs_[i]->uses().empty());
  op_ = nullptr;
  Value* n = outputs_[i];
...
void Node::eraseBlock(size_t i) {
  // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
  // GAUDI_JIT_ASSERT(i < blocks_.size());
  op_ = nullptr;
  Block* n = blocks_[i];
...
void Node::replaceAllUsesWith(Node* n) {
  // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
  // GAUDI_JIT_ASSERT(outputs().size() == n->outputs().size());
...
Value* Node::insertInput(size_t i, Value* value) {
  // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
  // GAUDI_JIT_ASSERT(graph_ == value->owningGraph());
  op_ = nullptr;
...
Value* Node::replaceInput(size_t i, Value* newValue) {
  // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
  // GAUDI_JIT_ASSERT(newValue->owningGraph() == graph_);
  op_ = nullptr;
...
void Node::replaceInputWith(Value* from, Value* to) {
  // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
  // GAUDI_JIT_ASSERT(from->owningGraph() == graph_);
  // GAUDI_JIT_ASSERT(to->owningGraph() == graph_);
  op_ = nullptr;
  size_t i = 0;
...
bool Node::isBeforeOrAfter(const Node* n, MoveSide moveSide) const {
  ...
  while (lhs) {
    // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
    // GAUDI_JIT_ASSERT(lhs->owningBlock());
  ...
  // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
  // GAUDI_JIT_ASSERT(false);
  // then remove return below, use proper assert instead of return false; for
...
Node* Node::insertBefore(Node* n) {
  // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
  // GAUDI_JIT_ASSERT(n->inBlockList());
  insertAfter(n->prev());
  return this;
}
...
Node* Node::insertAfter(Node* n) {
  // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
  // GAUDI_JIT_ASSERT(!inBlockList() && n->inBlockList());
  // GAUDI_JIT_ASSERT(n->owningBlock());
  // GAUDI_JIT_ASSERT(
  //     n->kind() != prim::Return,
  //     "Attempting to insert a Node after the Return node or before the Param
  //     node. Tried to insert", *this, " after ", *n,
  //     ".");
...
void Node::permuteInputs(const std::vector<size_t>& new_order) {
  op_ = nullptr;
  // GAUDI_JIT_ASSERT(new_order.size() == inputs_.size());
  std::vector<Value*> new_inputs;
  new_inputs.reserve(new_order.size());
  for (const auto i : c10::irange(new_order.size())) {
    // GAUDI_JIT_ASSERT(inputs_.at(new_order[i]) != nullptr, "Repeated index");
...
void Node::permuteOutputs(const std::vector<size_t>& new_order) {
  op_ = nullptr;
  // GAUDI_JIT_ASSERT(new_order.size() == outputs_.size());
  std::vector<Value*> new_outputs;
  new_outputs.reserve(new_order.size());
  for (const auto i : c10::irange(new_order.size())) {
    // GAUDI_JIT_ASSERT(outputs_.at(new_order[i]) != nullptr, "Repeated index");
...
use_list::iterator Node::findUseForInput(size_t i) {
  ...
  // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
  // GAUDI_JIT_ASSERT(use_it != input_uses.end());
  return use_it;
}
// todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
Value* Node::dropInput(size_t i) {
  // GAUDI_JIT_ASSERT(i < inputs_.size());
...
Block* Node::findCommonAncestorBlockWith(Node* n) {
  ...
  while (true) {
    ...
    n2 = n2->owningBlock()->owningNode();
    // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
    // GAUDI_JIT_ASSERT(n1 != nullptr);
    // GAUDI_JIT_ASSERT(n2 != nullptr);
  }
}
...
Node* Graph::createDict(
    ...
    at::ArrayRef<Value*> values) {
  // GAUDI_JIT_ASSERT(keys.size() == values.size());
  auto n = create(prim::DictConstruct, 1);
  for (const auto i : c10::irange(keys.size())) {
    // GAUDI_JIT_ASSERT(matchTypes(keys[i]->type(), key_type));
    // GAUDI_JIT_ASSERT(matchTypes(values[i]->type(), value_type));
...
void Graph::freeNode(Node* n) {
  ...
  // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
  // GAUDI_JIT_ASSERT(it != all_nodes.end());
  delete *it;
  all_nodes.erase(it);
}
void Graph::freeValue(Value* v) {
  ...
  // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
  // GAUDI_JIT_ASSERT(it != all_values.end());
  delete *it;
  all_values.erase(it);
}
void Graph::freeBlock(Block* b) {
  ...
  // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
  // GAUDI_JIT_ASSERT(it != all_blocks.end());
  delete *it;
  all_blocks.erase(it);
}
...
std::vector<Value*> insertGraph(
  ...
  // todo: use bridge infra https://jira.habana-labs.com/browse/SW-200787
  // GAUDI_JIT_ASSERT(callee.inputs().size() == inputs.size());
  for (const auto i : c10::irange(inputs.size())) {
