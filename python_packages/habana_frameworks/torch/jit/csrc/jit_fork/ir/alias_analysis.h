...
class AliasDb {
 public:
  ...
  static bool isMutableType(const c10::TypePtr& type);
 ...
 private:
  ...
  bool isMutableTypeInternal(const c10::TypePtr& type) const;
  ...
  const AliasTypeSet* mapTypeToAliasTypeSetPtr(const c10::TypePtr& type) const;
  ...
  ska::flat_hash_map<c10::TypePtr, Element*, HashType, EqualType>
      wildcardIndex_;
  Element* getWildcard(const c10::TypePtr& type) const;
  std::optional<Element*> tryGetOrCreateWildcard(const c10::TypePtr& type);
  ...
  mutable ska::flat_hash_map<c10::TypePtr, AliasTypeSet> mapped_mutable_types_;
