...
#include "jit_fork/passes/utils/subgraph_utils.h"
...
c10::MaybeOwned<c10::TypePtr> toSingleType(const AliasTypeSet& mut_types) {
  return mut_types.size() == 1
      ? c10::MaybeOwned<c10::TypePtr>::borrowed(mut_types[0])
      : c10::MaybeOwned<c10::TypePtr>::owned(c10::UnionType::create(mut_types));
}
...
class MutableTypePtrHelper {
 public:
  explicit MutableTypePtrHelper(
      ska::flat_hash_map<c10::TypePtr, AliasTypeSet>* mutable_type_cache)
  ...
  std::optional<AliasTypeSet> mapTypeToAliasTypeSet(const c10::TypePtr& type) {
    if (mutable_type_cache_ != nullptr) {
  ...
  const AliasTypeSet* mapTypeToBorrowedAliasTypeSet(const c10::TypePtr& type) {
 ...
 private:
  std::optional<AliasTypeSet> mapTypeToAliasTypeSetImpl(
    ...
    switch (type->kind()) {
      ...
      case TypeKind::TupleType: {
        std::vector<c10::TypePtr> mutable_types;
        for (const c10::TypePtr& inner :
             type->expectRef<TupleType>().elements()) {
          if (auto maybe_inner_types = mapTypeToAliasTypeSet(inner)) {
            mutable_types.insert(
                mutable_types.end(),
                (*maybe_inner_types).begin(),
  ...
  ska::flat_hash_map<c10::TypePtr, AliasTypeSet>* mutable_type_cache_;
};
...
bool AliasDb::isMutableType(const c10::TypePtr& type) {
...
bool AliasDb::isMutableTypeInternal(const c10::TypePtr& type) const {
...
void AliasDb::getWritesImpl(Node* n, MemoryLocations& ret) const {
  if (writeIndex_.has_value() && writeIndex_->count(n) != 0U) {
...
std::string AliasDb::toString() const {
  ...
  for (const auto& ptrPair : elementMap_) {
    auto* const element = ptrPair.second;
    int ct = 0;
...
bool AliasDb::tryRegisteredAnalysis(Node* node) {
  ...
  if (c10::AliasAnalysisKind::PURE_FUNCTION == analysis) {
    analyzeCreator(node);
    return true;
  }
  return false;
...
void AliasDb::analyzeImpl(Node* node) {
  ...
  const bool hasSpecialCase =
      torch::jit::aliasAnalysisHasSpecialCaseFor(node->kind());
  if (op != nullptr) {
    const auto analysis = op->aliasAnalysisKind();

    const bool registeredAsSpecialCase =
        analysis == c10::AliasAnalysisKind::INTERNAL_SPECIAL_CASE;
    if (C10_UNLIKELY(registeredAsSpecialCase && !hasSpecialCase)) {
      HABANA_ASSERT(
          false,
          "Op ",
          node->kind().toDisplayString(),
          " is registered with c10::AliasAnalysisKind::INTERNAL_SPECIAL_CASE but doesn't have a special case.");
    } else if (C10_UNLIKELY(!registeredAsSpecialCase && hasSpecialCase)) {
      HABANA_ASSERT(
          false,
          "Op ",
          node->kind().toDisplayString(),
          " has a special case and should be registered with c10::AliasAnalysisKind::INTERNAL_SPECIAL_CASE but is registered with ",
    ...
    if (!hasSpecialCase) {
      ...
      oss << "\n\nCandidates:";
      const auto& candidates = torch::jit::getAllOperatorsFor(node->kind());
  ...
  switch (node->kind()) {
    ...
    case prim::CallMethod: {
      ...
      // // so for each analysis of a particular function call we need a new
      // graph
      // // for all copies made, store them for duration of analysis so we do
      // not
  ...
  HABANA_ASSERT(op, "We should have an op schema if we get to here");
  const c10::AliasAnalysisKind analysis = op->aliasAnalysisKind();
  HABANA_ASSERT(
      analysis != c10::AliasAnalysisKind::INTERNAL_SPECIAL_CASE &&
          !torch::jit::aliasAnalysisHasSpecialCaseFor(node->kind()),
  ...
  if (node->kind().is_aten() || node->kind().is_prim() ||
    ...
    // ops using c10::AliasAnalysisKind::FROM_SCHEMA or
    // c10::AliasAnalysisKind::INTERNAL_SPECIAL_CASE, but this is the intended
    ...
    HABANA_ASSERT(
        analysis == c10::AliasAnalysisKind::FROM_SCHEMA ||
            analysis == c10::AliasAnalysisKind::CONSERVATIVE,
        "aten:: and prim:: operators should use c10::AliasAnalysisKind::FROM_SCHEMA or "
        "c10::AliasAnalysisKind::CONSERVATIVE(if really necessary), but ",
        node->kind().toDisplayString(),
        " doesn't. Note: Ideally, prim:: operators actually shouldn't have a schema ",
        "and then use c10::AliasAnalysisKind::INTERNAL_SPECIAL_CASE instead.");
  }

  if (analysis == c10::AliasAnalysisKind::CONSERVATIVE) {
  ...
  HABANA_ASSERT(
      analysis == c10::AliasAnalysisKind::FROM_SCHEMA,
      "c10::AliasAnalysisKind::CONSERVATIVE/PURE_FUNCTION/INTERNAL_SPECIAL_CASE should already have been handled above");
  ...
  for (const auto i : c10::irange(schema.returns().size())) {
    auto* const actual = node->outputs().at(i);
    ...
    for (const auto& formalAlias : formal->beforeSets()) {
      if (formalToActual.count(formalAlias) != 0U) {
...
void AliasDb::analyzeIf(Node* node) {
  ...
  auto* const trueBlock = node->blocks().at(0);
  auto* const falseBlock = node->blocks().at(1);
  analyze(trueBlock);
  analyze(falseBlock);

  for (const auto i : c10::irange(node->outputs().size())) {
    auto* const nodeOutput = node->outputs()[i];

    auto* const trueOutput = trueBlock->outputs().at(i);
    auto* const falseOutput = falseBlock->outputs().at(i);
...
void AliasDb::analyzeLoop(Node* node) {
  auto* const bodyBlock = node->blocks().at(0);
  ...
  HABANA_ASSERT(loopCarriedInputs.size() == blockInputs.size());
  HABANA_ASSERT(blockOutputs.size() == node->outputs().size());
...
void AliasDb::analyzeGradOf(Node* node) {
  auto* const grad_of_block = node->blocks().at(0);
...
void AliasDb::analyzeSubgraph(
  ...
  auto* const subgraphBlock = subgraph->block();
  ...
  // subgraph block.
  HABANA_ASSERT(subgraphBlock->outputs().size() >= node->outputs().size());
...
void AliasDb::analyzeWait(Node* node) {
  HABANA_ASSERT(node->kind() == aten::wait);
  for (auto* const output : node->outputs()) {
    setWildcard(output);
  }
...
void AliasDb::analyzeAwaitableWait(Node* node) {
  HABANA_ASSERT(node->kind() == prim::awaitable_wait);
  for (auto* const output : node->outputs()) {
    setWildcard(output);
  }
...
void AliasDb::analyzeSetAttr(Node* node) {
  auto* const self = node->inputs().at(0);
  HABANA_ASSERT(self->type()->kind() == TypeKind::ClassType);
  registerWrite(self, node);
  // Also the value being set must become a wildcard.
  auto* const newValue = node->inputs().at(1);
...
bool AliasDb::functionalNonEscapingListUse(const Use& use) const {
  ...
  */
  ...
  const auto* op = use.user->maybeOperator();
  return (
      op != nullptr &&
      op->aliasAnalysisKind() == c10::AliasAnalysisKind::PURE_FUNCTION);
}
...
void AliasDb::analyzeBroadcastingChunk(Node* node) {
  ...
  for (const auto index : c10::irange(inputs.size())) {
    ...
    const auto* output_begin = outputs.begin() + (index * nchunks);
    for (const auto* it = output_begin; it != output_begin + nchunks; ++it) {
...
void AliasDb::mapAliases(at::ArrayRef<Value*> from, at::ArrayRef<Value*> to) {
  HABANA_ASSERT(to.size() == from.size());
...
void AliasDb::createValue(const Value* value) {
  HABANA_ASSERT(isMutableTypeInternal(value->type()));
...
void AliasDb::giveFreshAlias(
  ...
  const auto* maybe_mut_types = mapTypeToAliasTypeSetPtr(value->type());
  if (maybe_mut_types == nullptr) {
    return;
  }

  if (elementMap_.count(value) != 0U) {
...
class AliasDb::WorkingSet {
 public:
  explicit WorkingSet(Node* mover, const AliasDb& aliasDb)
    ...
    for (auto* const user : getUsersSameBlock(mover_)) {
  ...
  void add(Node* n) {
    ...
    for (auto* const user : getUsersSameBlock(n)) {
      users_.insert(user);
    }
  ...
  bool dependsOn(Node* n) const {
    if (mover_ == nullptr && nodes_.empty()) {
      return false;
    }
 ...
 private:
  bool hasDataDependency(Node* n) const {
    ...
    const Node* pivot = mover_ != nullptr ? mover_ : nodes_.front();
    if (n->isAfter(pivot)) {
  ...
  bool hasMutabilityDependency(Node* n) const {
    ...
    if (mover_ != nullptr && moverReads_.intersects(nWrites)) {
      return true;
    }
    ...
    if (mover_ != nullptr && moverWrites_.intersects(nReads)) {
      return true;
    }
    return false;
  }
  ...
  bool producesFor(Node* n) const {
    ...
    if (mover_ != nullptr && moverUsers_.count(n) != 0U) {
      return true;
    }
  ...
  bool consumesFrom(Node* n) const {
    ...
    if (mover_ != nullptr && users.count(mover_) != 0U) {
      return true;
    }
...
bool AliasDb::tryMove(
  ...
  // 3. Execute the move
  HABANA_ASSERT(curNode == movePoint);
  if (splitToMoveAndDeps) {
    // Move `toMove`
...
std::optional<Element*> AliasDb::tryGetOrCreateWildcard(
    const c10::TypePtr& type) {
  const auto* maybe_mut_types = mapTypeToAliasTypeSetPtr(type);
  if (maybe_mut_types == nullptr) {
    return std::nullopt;
  }
...
void AliasDb::pointUnionTypeElementToAllContainedTypes(
  ...
  for (const auto& mut_type : mut_types) {
    ...
    if (maybe_elem) {
      HABANA_ASSERT(*maybe_elem != container_elem);
...
Element* AliasDb::getWildcard(const c10::TypePtr& type) const {
  const auto* maybe_mut_types = mapTypeToAliasTypeSetPtr(type);
  if (maybe_mut_types == nullptr) {
    return {};
  }
  if (maybe_mut_types->size() > 1) {
    ...
    // Get a <c10::TypePtr, Element*> pair where the TypePtr is this Union
    ...
    // Get a <c10::TypePtr, Element*> pair where the TypePtr is the given
    // type and the Element is the corresponding Wildcard
