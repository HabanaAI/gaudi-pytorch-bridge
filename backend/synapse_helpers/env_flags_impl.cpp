...
bool check_enum(
  ...
  while (std::getline(ss, token, CONSTRAINTS_SPLIT_PIPE)) {
    ...
    allowed_values.push_back(std::string(absl::StripAsciiWhitespace(token)));
  }
...
static std::vector<std::string> split_csv(const std::string& str) {
  ...
  std::stringstream ss(std::string(absl::StripAsciiWhitespace(str)));
  std::string item;
  while (std::getline(ss, item, CONSTRAINTS_SPLIT_COMMA)) {
    if (!absl::StripAsciiWhitespace(item).empty())
      result.push_back(std::string(absl::StripAsciiWhitespace(item)));
  }
  return result;
}
...
bool check_list(
  ...
  std::string ordered_part = std::string(absl::StripAsciiWhitespace(constraints.substr(0, comma_pos)));
  std::string list_part = std::string(absl::StripAsciiWhitespace(constraints.substr(comma_pos + 1)));
  ...
  if (ordered) {
    ...
    for (const auto& item : input) {
      if (std::none_of(allowed.begin(), allowed.end(), [&](const std::string& allowed_item) {
        return allowed_item == item;
        })) {
 ...
 * it meets security and structural requirements. It can validate whether the path
 * is a file or directory, check for unsafe or insecure characters, and optionally
 * create directories if specified.
 *
 * @param path The file or directory path to validate.
 * @param constraints A string containing constraint keywords and values. Supported keywords:
 ...
 * @param error_msg A reference to a string where error messages will be stored if validation fails.
 *
 * @return `true` if the path meets the constraints, `false` otherwise.
 *
 * @note If the path is empty, the function will log a warning and return `true`.
 * @note Unsafe and insecure characters in the path are checked using helper functions
 *       `has_unsafe_chars` and `has_insecure_chars`.
 * @note If the "is_dir" constraint is specified and the directory does not exist,
 *       it will be created if "is_create=true" is provided in the constraints.
...
bool check_file_path(
  ...
  bool is_create = get_value("is_create").value_or(CONSTRAINTS_FALSE) == CONSTRAINTS_TRUE;
...
bool check_recipe_cache_config(
  ...
  if (!std::regex_match(config, match, pattern)) {
    error_msg = config + " does not match expected format. Expected: <path>[,<true|false>][,<int_size_in_MB>][,<true|false>].";
    return false;
  }

  const std::string dir_path = std::string(absl::StripAsciiWhitespace(match[1].str()));
  if (!dir_path.empty()) {
    ...
    if (!fs::exists(dir_path)) {
      ...
      if ((fs::status(dir_path).permissions() & fs::perms::owner_write) == fs::perms::none) {
...
bool check(
  ...
  const std::string value_trim = std::string(absl::StripAsciiWhitespace(value_str));
  ...
  if (!constrainsType_str.empty()) {
    ...
    const std::string constraints_trim = std::string(absl::StripAsciiWhitespace(constraints_str));
    switch (type) {
