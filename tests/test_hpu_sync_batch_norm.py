...
import numpy as np
<<<<<<< HEAD:tests/test_hpu_sync_batch_norm.py
...
>>>>>>> 1c8e3092c... [SW-140881] python tests for PT, part 6:tests/pytest_working/test_hpu_sync_batch_norm.py
...
def batch_norm_backward_elemt_ref(
    grad_out, inp, mean, invstd, weight, sum_dy, sum_dy_xmu, counts
) -> torch.tensor:
    ...
    grad_in = (
        grad_out - (sum_dy_reshaped / total_count) - (inp - mean_reshaped) * factor_1_c
    ) * factor_2_c
...
def batch_norm_gather_stats_with_counts_ref(
    ...
    # TPC kernel mult
    partial_mean = (
        torch.cumsum(mean_times_counts, dim=0) * one_div_counts_accum_inclusive
    )
...
def test_all():
  ...
  for i in range(len(inp_split_list)):
    ...
    err_mean = np.allclose(hpu_mean_tensor.to(cpu).detach().numpy(), cpu_mean_tensor.detach().numpy(), atol=atol, rtol=rtol, equal_nan=True)

    cpu_invstd_tensor = invstd_list[i]
    hpu_invstd_tensor = invstd_list_hpu[i]
    err_invstd = np.allclose(hpu_invstd_tensor.to(cpu).detach().numpy(), cpu_invstd_tensor.detach().numpy(), atol=atol, rtol=rtol, equal_nan=True)
  ...
  g_mean, g_invstd, running_mean, running_var = batch_norm_gather_stats_with_counts_ref(torch.stack(mean_list), torch.stack(invstd_list), counts, running_mean, running_var, momentum, 1e-5)

  # HPU
  counts_hpu = counts.to(hpu)
  g_mean_hpu, g_invstd_hpu = torch.batch_norm_gather_stats_with_counts(inp_hpu, torch.stack(mean_list_hpu), torch.stack(invstd_list_hpu), running_mean_hpu, running_var_hpu, momentum, 1e-5, counts_hpu)
  # Verify runnig mean and variance
  err_mean = np.allclose(running_mean_hpu.to(cpu).detach().numpy(), running_mean.detach().numpy(), atol=atol, rtol=rtol, equal_nan=True)
  err_invstd = np.allclose(running_var_hpu.to(cpu).detach().numpy(), running_var.detach().numpy(), atol=atol, rtol=rtol, equal_nan=True)
  ...
  # Verify CPU and HPU results
  err_mean = np.allclose(g_mean_hpu.to(cpu).detach().numpy(), g_mean.detach().numpy(), atol=atol, rtol=rtol, equal_nan=True)
  err_invstd = np.allclose(g_invstd_hpu.to(cpu).detach().numpy(), g_invstd.detach().numpy(), atol=atol, rtol=rtol, equal_nan=True)
  ...
  for i in range(len(out_list)):
    err = np.allclose(out_list_hpu[i].to(cpu).detach().numpy(), out_list[i].detach().numpy(), atol=atol, rtol=rtol, equal_nan=True)
  ...
  for i in range(len(grad_out_list)):
    sum_dy, sum_dy_xmu, g_w, g_b = batch_norm_backward_reduce_ref(grad_out_list[i], inp_split_list[i], g_mean, g_invstd, None, True, False, False)
    sum_dy_hpu, sum_dy_xmu_hpu, g_w_hpu, g_b_hpu = torch.batch_norm_backward_reduce(grad_out_hpu_list[i], inp_split_list_hpu[i], g_mean_hpu, g_invstd_hpu, None, True, False, False)
  ...
  for i in range(len(grad_out_list)):
    err_sum_dy = np.allclose(sum_dy_hpu_list[i].to(cpu).detach().numpy(), sum_dy_list[i].detach().numpy(), atol=atol, rtol=rtol, equal_nan=True)
    err_sum_dy_xmu = np.allclose(sum_dy_xmu_hpu_list[i].to(cpu).detach().numpy(), sum_dy_xmu_list[i].detach().numpy(), atol=atol, rtol=rtol, equal_nan=True)
  ...
  print(f"global sum_dy rms error allclose = {np.allclose(g_sum_dy_hpu.to(cpu).detach().numpy(), g_sum_dy.detach().numpy(), atol=atol, rtol=rtol, equal_nan=True)}")
  print(f"global sum_dy_xmu rms error allclose = {np.allclose(g_sum_dy_xmu_hpu.to(cpu).detach().numpy(), g_sum_dy_xmu.detach().numpy(), atol=atol, rtol=rtol, equal_nan=True)}")
  print("-----")
  # Batch norm backward elemt
  grad_in_list = []
  grad_in_hpu_list = []
  for i in range(len(inp_split_list)):
    grad_in = batch_norm_backward_elemt_ref(grad_out_list[i], inp_split_list[i], g_mean, g_invstd, None, g_sum_dy, g_sum_dy_xmu, counts)
    grad_in_hpu = torch.batch_norm_backward_elemt(grad_out_hpu_list[i], inp_split_list_hpu[i], g_mean_hpu, g_invstd_hpu, None, g_sum_dy_hpu, g_sum_dy_xmu_hpu, counts_hpu.to(torch.int32))
...
def test_all():
    ...
    for i in range(len(inp_split_list)):
        ...
        # hpu
        dev_mean_hpu, dev_invstd_hpu = torch.batch_norm_stats(
            inp_split_list_hpu[i], 1e-5
        )
        mean_list_hpu.append(dev_mean_hpu)
        invstd_list_hpu.append(dev_invstd_hpu)
    ...
    (
        ...
        running_mean,
        running_var,
    ) = batch_norm_gather_stats_with_counts_ref(
        torch.stack(mean_list),
        torch.stack(invstd_list),
        counts,
        running_mean,
        running_var,
        momentum,
    ...
    # Verify runnig mean and variance
    err_mean = np.allclose(
        running_mean_hpu.to(cpu).detach().numpy(),
        running_mean.detach().numpy(),
        atol=atol,
        rtol=rtol,
        equal_nan=True,
    )
    err_invstd = np.allclose(
        running_var_hpu.to(cpu).detach().numpy(),
        running_var.detach().numpy(),
        atol=atol,
        rtol=rtol,
        equal_nan=True,
    )
    print(
        "Comparing results for batch_norm_gather_stats stage - Running mean and Running Variance"
    )
    print(
        f"Running Mean allclose = {err_mean}, Running Variance allclose = {err_invstd}"
    )

    # Verify CPU and HPU results
    err_mean = np.allclose(
        g_mean_hpu.to(cpu).detach().numpy(),
        g_mean.detach().numpy(),
        atol=atol,
        rtol=rtol,
        equal_nan=True,
    ...
    err_invstd = np.allclose(
        g_invstd_hpu.to(cpu).detach().numpy(),
        g_invstd.detach().numpy(),
        atol=atol,
        rtol=rtol,
        equal_nan=True,
    )
    ...
    out_list_hpu = []
    for i in range(len(inp_split_list)):
        out = batch_norm_elemt_ref(
            inp_split_list[i], None, None, g_mean, g_invstd, 1e-5
        )
        out_hpu = torch.batch_norm_elemt(
            inp_split_list_hpu[i], None, None, g_mean_hpu, g_invstd_hpu, 1e-5
        )
        out_list.append(out)
        out_list_hpu.append(out_hpu)
    ...
    for i in range(len(out_list)):
        err = np.allclose(
            out_list_hpu[i].to(cpu).detach().numpy(),
            out_list[i].detach().numpy(),
            atol=atol,
            rtol=rtol,
            equal_nan=True,
        )
    ...
    out = batch_norm_elemt_ref(inp, None, None, g_mean, g_invstd, 1e-5)
    out_hpu = torch.batch_norm_elemt(
        inp_hpu, None, None, g_mean_hpu, g_invstd_hpu, 1e-5
    )
    err = np.allclose(
    ...
    for i in range(len(grad_out_list)):
        ...
        print(
            f"Device: {i}: rms sum_dy allclose = {err_sum_dy}, rms sum_dy_xmu allclose = {err_sum_dy_xmu}"
        )
    print("-----")
    # All reduce
    sum_dy_tensor = torch.stack(sum_dy_list)
