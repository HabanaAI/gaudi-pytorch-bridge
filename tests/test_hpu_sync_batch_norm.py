...
def batch_norm_backward_reduce_ref(grad_out, inp, mean, invstd, weight, b_inp_grad, b_wei_grad, b_bias_grad) -> (torch.tensor, torch.tensor, torch.tensor, torch.tensor):
  grad_out_reshaped = grad_out.reshape(grad_out.shape[0], grad_out.shape[1], -1) # collapse H,W or H,W,K dims into one
...
def batch_norm_gather_stats_with_counts_ref(mean_tensor, invstds_tensor, count_tensor, running_mean, running_var, momentum, eps) -> (torch.tensor):
  # TPC kernel reshape
  ...
  one_div_counts_accum_inclusive = 1. / counts_accum_inclusive
  ...
  second_term = (tmp_partial_mean - mean_tensor) * (tmp_partial_mean - mean_tensor) * counts_accum_exclusive * counts_reshaped * one_div_counts_accum_inclusive
  # TPC kernel reciprocal
  v = 1 / invstds_tensor
