...
def test_hpu_transpose(N, H, W, C, t_op, kernel_params_fwd):
    print('In T = {}'.format(kernel_params_fwd['input']))
    print('Expected Out T = {}'.format(torch.transpose(kernel_params_fwd['input'],
                                        kernel_params_fwd['dim0'],kernel_params_fwd['dim1'])))
...
def test_hpu_transpose_of_transpose(N, H, W, C):
    ...
    thputensor = hputensor.transpose_(0, 2)
    tthputensor = thputensor.transpose_(0, 2)
    print (torch.eq(hputensor, tthputensor).to(cpu))
    

if __name__ == '__main__':
    test_hpu_transpose(*test_case_list[0],*t_op_list[0])
