...
test_list = ('all_reduce', 'all_reduce_multi_streams', 'all_reduce_comms_streams', 'all2all', 'all2all_v', 'all2all_v_custom', 'all_gather', 'reduce_scatter', 'read_write', 'all', 'loopback', 'sanity', 'compute_all_reduce')
...
def get_demo_cmd(id=0, size=1<<5, inner_loop=20000, loop=10, json_file='hls1.json', max_comm_id=0, a2av_custom_file=None,
                test='all_reduce', data_type='bfloat16', size_range=None, size_range_base=None, same_address=False,
                no_weak_order=False, skip_correctness=False, skip_performance=False,
                prof=None, enable_console='false', isTee=False, isLog=False, gdb=-1, num_cores_per_socket=18,
                num_sockets=2, num_hyper_threads=2, proc_affinity=False):
...
def run_process(p):
    ...
    print("Tensor values are same :: ",np.allclose(h2.detach().numpy(),
                     cpu_tensor.detach().numpy(), atol=0.001, rtol=1.e-3, equal_nan=True))
    stop_dl_workers(w)
...
def handle_args():
    ...
    parser.add_argument("--size", metavar="N", type=int,
                        help="Data size is 2^N")
    parser.add_argument("--size_in_bytes", metavar="N", type=int,
                        help="Data size is N")
    parser.add_argument("--size_range", type=int, nargs=2, metavar=("MIN","MAX"),
                        help="Test will run on all powers of 2 from 2^MIN to 2^MAX (must satisfy: 1 <= MIN < MAX <=32)")
    parser.add_argument("--size_range_base", type=int, nargs=2, metavar=("MIN","MAX"),
                        help="Test will run on all sizes from 2^MIN*num_ranks to 2^MAX*num_ranks (must satisfy: 1 <= MIN < MAX <=32)")
    parser.add_argument("--factor", metavar="N", type=int,
                        help="Factor is N (for 'all2all_v' test)")
    parser.add_argument("--loop", type=int,
                        help="Number of loop iterations")
    parser.add_argument("--max_comm_id", metavar="COMM_ID", type=int,
                        help="Max communicator ID from input JSON HCL_COMMUNICATORS section")
    parser.add_argument("--json", default="hls1.json", type=str,
                        help="Path to json config file (default is hls1.json)")
    parser.add_argument("--a2av_custom_file", type=str,
                        help="Path to configuration file for all2all_v_custom test")
    parser.add_argument("--data_type", metavar="TYPE", default="bfloat16", type=str,
                        help="TYPE=<float|bfloat16> (default=bfloat16)")
    parser.add_argument("--test", metavar="TEST", type=str,
                        help="Specify test (use '-l' option for test list)")
    parser.add_argument("-l", "--list_tests", action="store_true",
                        help="Show list of available tests")
    parser.add_argument("--same_address", action="store_true",
                        help="Use same address for RDMA operations")
    parser.add_argument("--no_weak_order", action="store_true",
    ...
    parser.add_argument("--no_correctness", action="store_true",
                        help="Skip correctness validation")
    parser.add_argument("--no_performance", action="store_true",
    ...
    parser.add_argument("--module_ids", type=int, nargs="*", choices=range(8),
                        help="List of Module IDs to assign per test process. List size must match the # of HCL ranks in JSON config file.")
    parser.add_argument("-g", "--gdb", type=int, default=-1,
                        help="Run process with gdb")
    parser.add_argument("-console", action="store_true",
                        help="Emit logs to terminal console (in addition to log file)")
    parser.add_argument("-prof", type=str, nargs="*", choices=[str(i) for i in range(8)] + ["all"],
    ...
    parser.add_argument("-tee", action="store_true",
                        help="Duplicate output to files: <i>.txt")
    parser.add_argument("-log", action="store_true",
                        help="Redirect output to files: log<i>.txt")
    parser.add_argument("-v", "--verbose", action="store_true",
                        help="Verbose logging from this wrapper script")
    # System CPU settings
    parser.add_argument("--num_cores_per_socket", metavar="CORES", type=int, default=28,
                        help="Number of cores per socket - default is 18")
    parser.add_argument("--num_sockets", metavar="SOCKETS", type=int, default=2,
                        help="Number of cpu sockets - default is 2")
    parser.add_argument("--num_hyper_threads", metavar="HYPER_THREADS", type=int, default=2,
                        help="Number of hyper threads per core - default is 2")
    parser.add_argument("--proc_affinity", action="store_true",
    ...
    if args.size_range:
        ...
        if  not (1 <= min_size_power <= 31) or\
            not (1 <= max_size_power <= 31) or\
            max_size_power <= min_size_power:
            # parse error
        ...
        if  not (1 <= min_size_base <= 30) or \
            not (1 <= max_size_base <= 30) or \
            max_size_base <= min_size_base:
            # parse error
