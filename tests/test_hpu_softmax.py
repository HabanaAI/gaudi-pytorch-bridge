...
test_case_list = [
    # N, H, W, C, dim
    pytest.param(2, 3, 4, 5, 1,
                 marks=pytest.mark.xfail(reason="SW-8560")),
]
...
def test_hpu_log_softmax_fwd_bwd(N, H, W, C, dim):
    in_tensors = [torch.randn(N, C, H, W), torch.randn(
        N, C, H, W), torch.randn(N, C, H, W)]
    evaluate_fwd_bwd_kernel(F.log_softmax, in_tensors)
