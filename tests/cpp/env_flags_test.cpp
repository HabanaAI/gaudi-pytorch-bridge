  ...
  EXPECT_THROW(
      {
        try {
          ...
          EXPECT_TRUE(
              std::string(e.what()).find(
                  "Flags name: PT_HPU_RECIPE_CACHE_CONFIG,/tmp/path,invalid,1024,false does not match expected format. Expected: <path>[,<true|false>][,<int_size_in_MB>][,<true|false>].") !=
              std::string::npos);
  ...
  EXPECT_THROW(
      {
        try {
          ...
          EXPECT_TRUE(
              std::string(e.what()).find(
                  "Flags name: PT_HPU_RECIPE_CACHE_CONFIG,/tmp/path,true,invalid,false does not match expected format. Expected: <path>[,<true|false>][,<int_size_in_MB>][,<true|false>].") !=
              std::string::npos);
  ...
  EXPECT_THROW(
      {
        try {
          ...
          EXPECT_TRUE(
              std::string(e.what()).find(
                  "Flags name: PT_HPU_RECIPE_CACHE_CONFIG,/tmp/path,true,1024,invalid does not match expected format. Expected: <path>[,<true|false>][,<int_size_in_MB>][,<true|false>].") !=
              std::string::npos);
...
TEST(FunctionEnvFlagsValidation, CheckEnum) {
  ...
  // Valid enum value
  EXPECT_TRUE(env_flags::new_style::check_enum(
      "value1", "value1|value2|value3", errorMsg));
  EXPECT_TRUE(errorMsg.empty());

  // Invalid enum value
  EXPECT_FALSE(env_flags::new_style::check_enum(
...
TEST(FunctionEnvFlagsValidation, CheckList) {
  ...
  // Valid ordered list
  EXPECT_TRUE(env_flags::new_style::check_list(
  ...
  EXPECT_FALSE(env_flags::new_style::check_list(
...
TEST(FunctionEnvFlagsValidation, CheckFilepath) {
  ...
  EXPECT_TRUE(env_flags::new_style::check_file_path(
  ...
  // Invalid path with unsafe characters
  EXPECT_FALSE(env_flags::new_style::check_file_path(
...
TEST(FunctionEnvFlagsValidation, CheckRecipeCacheConfig) {
  ...
  // Valid configuration
  EXPECT_TRUE(env_flags::new_style::check_recipe_cache_config(
      "/tmp/recipe-cache,true,1024,false", errorMsg));
  EXPECT_TRUE(errorMsg.empty());

  // valid format: missing 1 st field
  EXPECT_TRUE(env_flags::new_style::check_recipe_cache_config(
      ",true,1024,false", errorMsg));
  EXPECT_TRUE(errorMsg.empty());

  // valid format: missing 4 th field
  EXPECT_TRUE(env_flags::new_style::check_recipe_cache_config(
      "/tmp/recipe-cache,true,1024", errorMsg));
  EXPECT_TRUE(errorMsg.empty());

  // valid format: missing 1 st and 4 st field
  EXPECT_TRUE(env_flags::new_style::check_recipe_cache_config(
      ",true,1024", errorMsg));
  EXPECT_TRUE(errorMsg.empty());

  // Invalid directory path: contains unsafe characters
  EXPECT_FALSE(env_flags::new_style::check_recipe_cache_config(
