...
TEST(LazyKernelTest, MaxPoolBWDTest) {
  setenv("PT_HPU_LAZY_MODE", "1", 1);
  auto input_tensor = torch::arange(48, torch::dtype(torch::kFloat).requires_grad(true))
                          .reshape({1, 3, 4, 4}); //nchw
  torch::Tensor tHabanaX = input_tensor.to(torch::kHABANA);
  ...
  auto grad_tensor = torch::arange(27, torch::dtype(torch::kFloat).requires_grad(true))
                          .reshape({1, 3, 3, 3});
