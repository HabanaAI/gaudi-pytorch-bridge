...
TEST_F(LazyBasicKernelTest, DISABLED_permuteContCLTest) {
  torch::Tensor A =
...
TEST_F(LazyBasicKernelTest, DISABLED_SliceOnChlastInput) {
  torch::Tensor A =
...
TEST_F(LazyBasicKernelTest, DISABLED_SelectOnChlastInput) {
  torch::Tensor A =
      torch::randn({2, 4, 3, 5}).contiguous(c10::MemoryFormat::ChannelsLast);
  auto hA = A.to(torch::kHPU);
  auto B = torch::select(A, 3, 1);
...
TEST_F(LazyBasicKernelTest, DISABLED_InplaceSliceonChlast3d) {
...
TEST_F(LazyBasicKernelTest, viewclone) {
  torch::Tensor A = torch::randn({2, 2});
  auto hA = A.to(torch::kHPU);
  auto B = torch::clone(A.as_strided({2, 2}, {1, 2}));
  auto C = B.as_strided({2, 2}, {1, 2});

  auto hB = torch::clone(hA.as_strided({2, 2}, {1, 2}));

  auto hC = hB.as_strided({2, 2}, {1, 2});

  EXPECT_EQ(allclose(C, hC.cpu(), 0.001, 0.001), true);
}

TEST_F(LazyBasicKernelTest, viewclone2) {
  torch::Tensor A = torch::randn({2, 2, 2, 2});

  auto hA = A.to(torch::kHPU);
  auto B = torch::clone(A.as_strided({2, 2, 2, 2}, {8, 4, 1, 2}));
  auto C = B.as_strided({2, 2, 2, 2}, {8, 1, 4, 2});
  C.add_(1.0);

  auto hB = torch::clone(hA.as_strided({2, 2, 2, 2}, {8, 4, 1, 2}));

  auto hC = hB.as_strided({2, 2, 2, 2}, {8, 1, 4, 2});
  hC.add_(1.0);
...
TEST_F(LazyBasicKernelTest, viewclone3) {
  torch::Tensor A = torch::randn({2, 2, 2, 2});
  auto hA = A.to(torch::kHPU);
  auto B = torch::clone(A.as_strided({2, 2}, {2, 1}));
  auto C = B.as_strided({2, 2}, {1, 2});

  auto hB = torch::clone(hA.as_strided({2, 2}, {2, 1}));
