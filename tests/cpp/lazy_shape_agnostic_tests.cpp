...
TEST_F(LazyShapeAgnosticTest, DISABLED_LazyDoATest1) {
...
TEST_F(LazyShapeAgnosticTest, DISABLED_LazyDoATest2) {
  torch::Tensor A = torch::randn({5, 5}, torch::requires_grad(false));
  torch::Tensor B = torch::randn({5, 5}, torch::requires_grad(false));
  torch::Tensor hA = A.to(torch::kHPU);
  torch::Tensor hB = B.to(torch::kHPU);
...
TEST_F(LazyShapeAgnosticTest, DISABLED_LazyDoATest3) {
...
TEST_F(LazyShapeAgnosticTest, DISABLED_ConvReluTest1) {
  auto input_tensor =
...
TEST_F(LazyShapeAgnosticTest, DISABLED_ConvReluTest2) {
...
TEST_F(LazyShapeAgnosticTest, DISABLED_MulInplaceTest) {
...
TEST_F(LazyShapeAgnosticTest, DISABLED_BatchNormForwardExecute) {
  if (false == GET_ENV_FLAG_NEW(PT_HPU_VALIDATE_COMPUTE_SHAPE)) {
    SET_ENV_FLAG_NEW(PT_HPU_VALIDATE_COMPUTE_SHAPE, true, 1);
  }
  ...
  auto var_2 = torch::ones(5, torch::dtype(torch::kFloat).requires_grad(false));
...
TEST_F(LazyShapeAgnosticTest, DISABLED_LayerNormForwardExecute) {
  SET_ENV_FLAG_NEW(PT_HPU_ENABLE_LAZY_EAGER_EXECUTION_THREAD, false, 1);
  auto input_tensor =
...
TEST_F(LazyShapeAgnosticTest, DISABLED_AvgPoolTest) {
  auto input_tensor =
...
TEST_F(LazyShapeAgnosticTest, DISABLED_ConvTranspose2dBwdTest) {
