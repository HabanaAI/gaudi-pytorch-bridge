...
TEST(DSCacheTest, UniqueTokenGenTest) {
  auto t0 = habana::UniqueTokenGenerator::get_gen().token();
  std::unordered_set<uint64_t> S;
  S.insert(t0);

  for (int i = 0; i < 10000; i++) {
    auto tok = habana::UniqueTokenGenerator::get_gen().token();
    EXPECT_EQ(S.count(tok), 0);
    S.insert(tok);
  }
}

TEST(DSCacheTest, JIT_IR_GraphKeyTest) {
