...
@pytest.mark.parametrize("divisor_override", [None, 4, -3])
@pytest.mark.parametrize("dtype", [torch.float], ids=format_tc)
def test_hpu_avg_pool3d_bwd(shape, kernel_size_and_padding, stride, ceil_mode, count_include_pad, divisor_override, dtype):
    def fn(input):
        ...
        fwd.backward(grad)
        output = torch.ops.aten.avg_pool3d_backward(grad, input.detach(), kernel_size, stride, padding, ceil_mode, count_include_pad, divisor_override, grad_input=grad_input)
