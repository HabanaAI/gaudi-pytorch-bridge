...
def common_determinism(fn, input, dtype, op_name):
...
def test_bernoulli_determinism_one_graph():
    ...
    common_determinism(fn, input, torch.float, "habana_bernoulli")
...
def test_poisson_determinism():
    ...
    common_determinism(fn, input, torch.float32, "habana_poisson")


@pytest.mark.parametrize("dtype", [torch.float32, torch.bfloat16], ids=format_tc)
@pytest.mark.parametrize("is_like", [False, True])
def test_rand(dtype, is_like):
    shape = (2, 5, 6)
...
@pytest.mark.parametrize("low, high", [(-50, 20), (None, 10000)], ids=format_tc)
@pytest.mark.parametrize("is_like", [False, True])
@pytest.mark.parametrize("dtype", [torch.int, torch.long], ids=format_tc)
def test_randint(low, high, is_like, dtype):
    shape = (5, 10, 15)
...
def test_randperm(dtype):
    def fn(shape):
        return torch.randperm(shape, dtype=dtype, device="hpu")

    common_determinism(fn, 8, dtype, "habana_randperm")
...
def test_exponential(dtype):
    ...
    a = torch.empty((3, 4, 5), dtype=dtype).to("hpu")
...
def test_various_ops(dtype):
    def fn(input_a, input_b, shape_c, multinomial_input):
        ...
        efghij = torch.add(efghi, j)
        result = torch.addmm(ab, cd, efghij)
        return result
