...
@pytest.mark.parametrize("dtype", [torch.float32, torch.bfloat16, torch.int])
def test_kv_reorder(shape, dtype):
    ...
    if dtype == torch.bfloat16:
        input_hpu = input_hpu.to(torch.float)
        input_cpu = input_cpu.to(torch.float)
    np.testing.assert_equal(input_hpu.cpu().numpy(), input_cpu.numpy())
