...
def create_attention_mask_for_test(batch_size, n_heads, seq_len_N_t, seq_len_N_s, dtype, shape, float_mask = True): #RTC: cross attention will need to use source and target seq lens
...
def vanilla_attention_impl_for_test(query, key, value, attn_mask = None, dropout_p=0.0, is_causal = False, dbg_dropout_mask = None):
...
def perf_cmp_fsdpa_vs_vanilla_attn(g_hpu, q_hpu, k_hpu, v_hpu, attn_mask = None, dropout_p=0.0, is_causal = False):
    ...
    if check_dbg_env_var('FSDPA_DBG_PERF_CMP_RUN_FSDPA'):
        ...
        O_hpu = vanilla_attention_impl_for_test(q_hpu,k_hpu,v_hpu, attn_mask_hpu, dropout_p = dropout_p, is_causal = is_causal)
    else:
        is_perf_run = False
        return is_perf_run
...
def test_sdpa(
    ...
    if use_attn_mask:
        attn_mask = create_attention_mask_for_test(batch_size, n_heads, seq_len_N_t, seq_len_N_s, mask_dtype, attn_mask_shape, float_mask = use_float_mask)
    ...
    for i in range(perf_run_count) :
        ...
        perf_run = perf_cmp_fsdpa_vs_vanilla_attn(g_hpu, q_hpu, k_hpu, v_hpu, attn_mask = attn_mask_hpu, dropout_p=dropout_p)
        if profile_api and i == profile_step:
