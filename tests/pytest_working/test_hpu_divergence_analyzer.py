...
def test_divergence_tool(train_cmd, do_train, do_compare, to_csv, do_split, enable_parallel, outdir, exit_on_first_mismatch):
    n_devices = hpu.device_count()
    if enable_parallel == 1 and n_devices < 2:
        return

    expected_status = predict_status(train_cmd, do_train, do_compare, to_csv, do_split, enable_parallel, outdir, exit_on_first_mismatch)

    if do_train == 0 and do_compare == 1:
        cmd_train = get_cmd(train_cmd=TRAIN_CMD, do_train=1, do_compare=0, to_csv=0, do_split=do_split, enable_parallel=do_split, outdir=outdir, exit_on_first_mismatch=exit_on_first_mismatch)
        os.system(cmd_train)
