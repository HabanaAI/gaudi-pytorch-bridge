...
@pytest.mark.parametrize(
    "is_scale_tensor, is_quant_tensor", [(True, True), (True, False), (False, False)]
)
@pytest.mark.parametrize("dtype", [torch.float, torch.bfloat16])
@pytest.mark.parametrize("out_dtype", quant_dtypes)
def test_quantize_per_tensor(
    shape, scale, zero_point, is_scale_tensor, is_quant_tensor, dtype, out_dtype
):
    ...
    result_hpu = fn(
        input_hpu, scale_hpu, zero_point_hpu, quant_min_hpu, quant_max_hpu, out_dtype
    )
...
def test_quantize_per_channel(shape, axis, dtype, out_dtype):
    ...
    result_hpu = fn(
        input_hpu, scales_hpu, zero_points_hpu, axis, quant_min, quant_max, out_dtype
    )
    result_cpu = fn_ref(
        input, scales, zero_points, axis, quant_min, quant_max, out_dtype
    )
...
def test_dequantize_per_channel(shape, axis, dtype, orig_dtype):
    def fn(input, scales, zero_points, axis, orig_dtype):
        return torch.ops.quantized_decomposed.dequantize_per_channel(
            input, scales, zero_points, axis, 0, 0, orig_dtype
        )
