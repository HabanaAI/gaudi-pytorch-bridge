...
split_embedding_bag_test_case_list = [
  ...
  ( 36,                 [0, 12, 24],         [0, 4, 8, 12], 13,            3,
    [[0.4963, 0.7682, 0.0885, 0.1320, 0.0223, 0.1689, 0.2939, 0.5185, 0.8388, 1.1058, 1.9055, 0.0723],
     [0.4963, 0.7682, 0.0885, 0.1320, 0.0447, 0.3377, 0.5878, 1.0370, 0.4194, 0.5529, 0.9527, 0.0362],
     [0.9925, 1.5364, 0.1770, 0.2641, 0.0223, 0.1689, 0.2939, 0.5185, 0.8388, 1.1058, 1.9055, 0.0723]]
   ),
]

common_args_dict = dict(host_weights=torch.rand(36),
...
optimizer_args_dicts = {
    "sgd": OptimizerMeta(dict(gradient_clipping=False,
                              max_gradient=1.0,
                              stochastic_rounding=True,
                              learning_rate=0.01),
                         split_embedding_codegen_lookup_sgd_function_hpu),
    "adagrad": OptimizerMeta(dict(gradient_clipping=False,
                                  max_gradient=1.0,
                                  stochastic_rounding=True,
...
def test_split_embedding_bag(optimizer, host_weights_numel, weights_offsets, D_offsets, indices_numel, out_rows, expected_output_data):
    ...
    common_args_dict['indices'] = torch.randint(low=0,
                            high=min([
                                common_args_dict['weights_offsets'][idx]//common_args_dict['D_offsets'][idx + 1] for idx in range(1, len(weights_offsets))
                            ]), size=[indices_numel])
    ...
    output = split_embedding_codegen_lookup_function(
            **common_args,
            **optimizer_args
                    )
