...
def test_nms(num_boxes, iou_threshold):
    ...
    try:
        from hb_custom import HabanaNms
    except ImportError:
        raise ImportError("Please install hb_custom.")
    nms = HabanaNms()
    keep_hpu = nms.habana_nms(
        boxes.to(hpu), scores.to(hpu), iou_threshold, score_threshold
    )
    keep_hpu = nms.habana_nms(
        boxes.to(hpu), scores.to(hpu), iou_threshold, score_threshold
    )
...
def test_batched_nms(num_boxes, iou_threshold):
    ...
    keep_hpu = nms.habana_batched_nms(
        boxes.to(hpu), scores.to(hpu), idx.to(hpu), iou_threshold, score_threshold
    )
    compare_tensors(keep_hpu.to(cpu), keep_cpu, atol=0, rtol=0)
