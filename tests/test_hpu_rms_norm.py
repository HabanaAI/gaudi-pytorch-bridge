...
@pytest.mark.parametrize("dtype", [
    pytest.param(torch.float16, marks=[pytest.mark.skipif(is_gaudi1(), reason="Gaudi1 doesn't support fp16")]),
    pytest.param(torch.float32),
    pytest.param(torch.bfloat16)])
def test_rms_norm_fwd_case(D, W, H, B, eps, dtype):
    if D == 32 and W == 16 and H == 8 and B == 2 and dtype == torch.float32:
