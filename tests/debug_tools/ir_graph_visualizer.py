...
def create_get_node(onnx_nodes, name, op_type, root_name):
       if name not in onnx_nodes:
                node = h.make_node(op_type, name=name, inputs=[], outputs=[], doc_string=f'no info log line found or this op is not in the present path till/of : {root_name}')
...
def create_onnx_graph(graph):
     ...
     g = h.make_graph(nodes = list(onnx_nodes.values()), name='ir_graph_log', inputs = [], 
        outputs = [h.make_empty_tensor_value_info(root_name)], doc_string='visualization of ir log graph')
     return g
