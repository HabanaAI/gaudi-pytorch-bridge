...
def read_under_marker(file_name, mark):
    ...
    for line in read_reverse(file_name):
        if mark in line: break
        lines.insert(0, line)
        if MARKER in line: lines.clear()
    return lines
def extract_graph_logs(ir_lines : str):
    ...
    for line in ir_lines:
        ...
        if read: graph_log_list[-1].append(line)
        if line.startswith('return'): read = False
...
def build_var_table(graph_log : str):
    ...
    for line in graph_log:
        ...
        while i < len(line):
            if read_code == 1 and line[i] == '[' or line[i] == '(' : read_code  = 0  # sometimes names don't end with [] instead ()
            if line[i] == ')': break # end of args
            if line[i-1] == '(':
                 read_code = 2
        ...
        for name in curr_var_list:
            if op_name in opnamecount: opnamecount[op_name]+=1
            else: opnamecount[op_name] = 1
...
def parse_all_ops(graph_log : str, op : str, op_index):
    ...
    for val in var_table.values():
        if len(val[0]) == 0 or len(val[1]) == 0: continue
        neighbors = [(var_table[e][0], var_table[e][-1]) for e in val[1] if len(var_table[e][0]) > 0 and len(var_table[e][1]) > 0]
