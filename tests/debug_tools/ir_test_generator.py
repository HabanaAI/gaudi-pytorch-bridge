...
def process_info_line(info_line):
    ...
    for c in str_args:
        if not key_added and c != '=': key+=c
            ...
            if len(value) > 0: var_value_list[-1][-1].append(value)
            value = '{'
            brack_open = True
        elif key_added and c == ']':
            value+='}'
    ...
    var_value_list[-1][-1].append(value)
    extra = op_info.split(' ')
    if len(extra) > 1: op_info = extra[-1]
...
def get_neighbors(adj_node):
    ...
    except TypeError: return adj_node
...
def wrap_in_aux_method_depth(adj, path_nodes, mtype):
    ...
    for l, node in enumerate(reversed(path_nodes)):
        if node in NO_LOG_OP or node[0].split(":")[-1] in REMOVE_TEMP: raise SubGraphTestCaseException(f"node : {node} returns a non-tensor or no-info-log present")
        var_names = []
...
def generate_test_case_cumulative(graph , depth):
    ...
    for node, l in graph_path_gen(graph, True, True):
        if (depth - l) < 0: continue #not consider deeper nodes
        path_nodes.append(node)

    if len(path_nodes) == 0: return ""
...
if __name__ == '__main__':
    ...
    parser.add_argument('-op','--op_name', dest='op_name', help='op name like conv/relu/dropout etc', required=True, type = str)
    parser.add_argument('-idx','--op_index', dest='op_idx', help = 'search for op at {idx}, -1 / blank for last op occurence', default=-1, type = int)
    parser.add_argument('-m', '--marker_name', dest = 'marker', help='marker name from file to search from, example: <mark start : 0>...', required=True, type = str)
    ...
    parser.add_argument('-v', '--verbose', dest = 'verbose', help='Print Generated file and Adj matrices', action="store_true")
    parser.add_argument('-donnx', '--dest_onnx', dest = 'onnx_file_path', help = 'onnx destination file path', type = str, default=f'{home}/ir_log.onnx')
    parser.add_argument('-dtest', '--dest_test', dest = 'cpp_file_path', help = 'cpp test destination file path', type = str, default = f'{home}/sample_test.cpp')
    ...
    op_name, op_idx, marker, ilog, irlog, verbose = args['op_name'], args['op_idx'], args['marker'], args['info_log'], args['ir_log'], args['verbose']
