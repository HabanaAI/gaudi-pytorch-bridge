import math
<<<<<<< HEAD:tests/test_hpu_te_self_attention.py
import torch
import pytest
from test_utils import is_gaudi1
=======

>>>>>>> 1c8e3092c... [SW-140881] python tests for PT, part 6:tests/pytest_working/test_hpu_te_self_attention.py
import habana_frameworks.torch.hpex.experimental.transformer_engine as te
import pytest
import torch
from habana_frameworks.torch.hpex.experimental.transformer_engine import (
    SelfAttentionContext,
    SelfAttentionScoresAndValue,
    fp8_autocast,
)
...
class TESelfAttention(torch.nn.Module):
    ...
    def __init__(self, config) -> None:
        ...
        self.phase1 = SelfAttentionScoresAndValue(
            config.hidden_size, config.num_attention_heads
        )
        self.phase2 = SelfAttentionContext(
            config.hidden_size, config.num_attention_heads
        )
...
def cosine_similarity(
    tensor1: torch.Tensor, tensor2: torch.Tensor, return_angle=True
) -> float:
    cs_ = (
        tensor1.flatten()
        @ tensor2.flatten()
...
def prepare_fp8_recipe():
    ...
    fp8_format = (
        recipe.Format.E5M2
    )  # No stochastic rounding on bwd - for deterministic tests
    fp8_margin = 0
    fp8_interval = 1
...
def self_attention(
    ...
    config,
    device,
    self_attention_class=TESelfAttention,
...
def self_attention_n_times(
    inputs, config, device, self_attention_class=TESelfAttention
):
    batch = inputs[0].shape[0]
    qkv_dim = inputs[0].shape[1]
    te_attention_mask = torch.ones((batch, 1, qkv_dim, qkv_dim), device=device)
    ...
    for input in inputs:
        if self_attention_obj is None:
            out, grad, self_attention_obj = self_attention(
                ...
                config,
                device,
                self_attention_class=self_attention_class,
            )
        else:
            out, grad, self_attention_obj = self_attention(
                input,
                ...
                config,
                device,
                self_attention_obj=self_attention_obj,
            )
        outputs.append(out)
        grads.append(grad)
...
def test_bert_self_attention():
    ...
    ref_input = torch.ones(
        (batch, qkv_dim, config.hidden_size),
        device=device,
        requires_grad=True,
        dtype=torch.bfloat16,
    )
    ref_attention_mask = torch.ones((batch, 1, qkv_dim, qkv_dim), device=device)
    ...
    out_ref, grad_ref, _ = self_attention(
        ...
        config,
        device,
        self_attention_class=BertSelfAttention,
    )
