...
def evaluate_fwd_kernel(kernel, tensor_list, kernel_params={}, check_results=True):
    ...
    hpu_result = _run_kernel_on_device(hpu, _kernel_copy_to_device(
        kernel, hpu), hpu_tensor_list, kernel_params)
...
def evaluate_fwd_bwd_kernel(kernel, tensor_list_fwd, tensor_list_bwd, kernel_params={}, check_results_fwd=True, check_results_bwd=True):
    ...
    hpu_result_fwd = _run_kernel_on_device(
        hpu, kernel, hpu_tensor_list_fwd, kernel_params)
...
def compare_tensors(hpu_tensors, cpu_tensors, atol, rtol):
    ...
    for i in range(len(hpu_tensors)):
        np.testing.assert_allclose(hpu_tensors[i].to(cpu).detach(
        ).numpy(), cpu_tensors[i].detach().numpy(), atol=atol, rtol=rtol)
