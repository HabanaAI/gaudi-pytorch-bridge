...
def test_hpu_pool_chlast_fwd_bwd(N, H, W, C, Ho, Wo):
    ...
    # don't check fwd results because indices can have different values
    (hpu_result_fwd, hpu_result_bwd), (cpu_result_fwd, cpu_result_bwd) = evaluate_fwd_bwd_kernel(kernel=kernel, tensor_list_bwd=bwd_tensors,
                                                                       kernel_params_fwd=kernel_params_fwd, check_results_fwd=False)
    compare_tensors(hpu_result_fwd[0], cpu_result_fwd[0], atol=0.001, rtol=1.e-3)
    compare_tensors(hpu_result_bwd[0], cpu_result_bwd[0], atol=0.001, rtol=1.e-3)
