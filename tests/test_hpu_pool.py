...
@pytest.mark.parametrize("N, H, W, C, R, S, str_H, str_W, padding, type, inpad, ceil_mode", pool_test_case_list)
...
def test_hpu_pool_fwd_bwd(N, H, W, C, R, S, str_H, str_W, padding, type, inpad, ceil_mode, dtype, tol):
    ...
    bwd_tensors = [torch.randn(N, C, output_size(H, padding, 1, R, str_H, ceil_mode), output_size(W, padding, 1, S, str_W, ceil_mode)).to(dtype)]
...
def test_hpu_pool_chlast_fwd_bwd(N, H, W, C, R, S, str_H, str_W, padding, type, inpad, ceil_mode):
    ...
    bwd_tensor = torch.randn(N, C, output_size(H, padding, 1, R, str_H, ceil_mode), output_size(W, padding, 1, S, str_W, ceil_mode))
