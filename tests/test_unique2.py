...
def _test_allclose(a: torch.Tensor, sorted: bool = True, dim: Union[int, None]=None, atol: float = 1e-8) -> None:
    if sorted:
        assert np.allclose(torch.unique(a.to("hpu"), sorted=sorted, dim=dim).cpu().numpy(), torch.unique(a, sorted=sorted, dim=dim).numpy(), atol=atol, equal_nan=True)
    else: # Result coming in habana is reverse order than CPU, reversing the CPU to match habana
        assert np.allclose(torch.unique(a.to("hpu"), sorted=sorted, dim=dim).cpu().numpy(), torch.flip(torch.unique(a, sorted=sorted, dim=dim), dims=(0,)).numpy(), atol=atol, equal_nan=True)
