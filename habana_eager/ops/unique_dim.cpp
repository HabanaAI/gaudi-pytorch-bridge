...
std::tuple<at::Tensor, at::Tensor, at::Tensor> unique_dim_eager(
  ...
  if (return_inverse && return_counts) {
    ...
    const auto result_unique = hpu_op.call();
    const auto feature_map = std::get<0>(result_unique);
    const auto valid_count = std::get<1>(result_unique);
    const auto end = valid_count.item<int64_t>();
