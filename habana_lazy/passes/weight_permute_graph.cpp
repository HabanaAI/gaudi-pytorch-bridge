...
bool isMulOutVarient(const torch::jit::Value* value) {
  for (auto u : value->uses()) {
    auto node = u.user;
    if ((strcmp(node->kind().toQualString(), "aten::mul") == 0) &&
        (node->inputs().size() == 3)) {
      return true;
    }
  }
...
bool isAsStridedNode(const torch::jit::Value* value_in) {
  if ((strcmp(
           value_in->node()->kind().toQualString(), "hpu::as_strided_lazy") ==
       0) ||
      (strcmp(value_in->node()->kind().toQualString(), "hpu::strided_view") ==
       0)) {
    return true;
  }
  return false;
...
bool isStridedNode(const torch::jit::Value* value_in) {
  bool flag = false;

  if ((strcmp(value_in->node()->kind().toQualString(), "hpu::strided_insert") ==
       0) ||
      (strcmp(value_in->node()->kind().toQualString(), "hpu::strided_view") ==
       0)) {
    flag = true;
  ...
  for (auto& use : value_in->uses()) {
    ...
    if ((strcmp(
             node_as_strided->kind().toQualString(), "hpu::strided_insert") ==
         0) ||
        (strcmp(node_as_strided->kind().toQualString(), "hpu::strided_view") ==
         0)) {
      flag = true;
    }
  }
...
void InsertWeightRestride_graph(
          ...
          if (strcmp(output_node->kind().toQualString(), "prim::ListUnpack") ==
            ...
            for (auto list_output_val : output_node->outputs()) {
              if (*list_output_val->type()->cast<TensorType>()->dim() == 4 ||
                  *list_output_val->type()->cast<TensorType>()->dim() == 5) {
  ...
  weight_pass.markWeightTensors(graph, false);
  auto weight_values = weight_pass.getWeightTensors();
    ...
    if (isInGraphInputs(graph, weight_value)) {
      ...
      if (stack[value_idx].isTensor()) {
        ...
        if (layout_format != habana_lazy::LayoutFormat::kHWCK) {
          ...
          auto op_as_strided1 =
              c10::Symbol::fromQualString("hpu::as_strided_layout");
          auto dims2 = getDimsForWeightLayout(
              habana::LayoutFormat::HWCK,
              habana::LayoutFormat::NCHW,
              tensor.dim());
          auto value_dims2 = graph->insertConstant(IValue(dims2));
          auto as_strided_node1 =
              graph->create(op_as_strided1, {value_in, value_dims2}, 1);
          as_strided_node1->insertAfter(permute_node);
          ...
          auto d2d_copy_node = graph->create(
              ...
              1);
          d2d_copy_node->insertAfter(control_edge_node2);
