...
class Convolution : public ir::Node {
 ...
 enum class ConvParams { BIAS_INDEX=2,
  ...
  Convolution(
      ...
      : Node(c10::Symbol::fromQualString("aten::convolution_backward_overrideable")) {
    ...
    m_meta_data.set(output_padding, static_cast<size_t>(ConvParams::OUTPUT_PADDING_INDEX));
    m_meta_data.set(groups, static_cast<size_t>(ConvParams::GROUPS_INDEX));
    m_meta_data.set(output_mask, static_cast<size_t>(ConvParams::OUTPUT_MASK_INDEX));
  }
  std::string ToString() const override {
    ...
    if(m_meta_data.count(static_cast<size_t>(ConvParams::OUTPUT_MASK_INDEX))) {
      ss << ", output_mask=" << m_meta_data.get(static_cast<size_t>(ConvParams::OUTPUT_MASK_INDEX));
    }
    return ss.str();
  }
};
