...
Tensor transpose_hpu(const Tensor& self, int64_t dim0_, int64_t dim1_) {
  ...
  /*Once the dimension sizes are swapped, the real layout will have strides as commented below
  * #if 0
  * int k;
  * self_strides[self_sizes.size() - 1] = 1;
  * for (k = self_strides.size() - 2; k >= 0; k--) {
  *   self_strides[k] = self_strides[k+1]*self_sizes[k+1];
  * }
  * #endif
  * Unfortunately, thats not how pytorch "views" transposed tensor dimensions.
  * It just takes a different 'view' of original tensor by swapping the strides.
  * So, we do it the PyTorch way and not the "real" way.
  */
  std::swap(self_strides[dim0], self_strides[dim1]);
