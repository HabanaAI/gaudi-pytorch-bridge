...
void habana::BinaryInplaceWrapperOperatorWithAlpha::AllocateAndAddSynapseNode(
  ...
  BinaryInplaceOperatorWithAlpha binaryOp(
...
void habana::BinaryInplaceOperator::insert_reshape_op(
    synapse_helpers::graph& graph,
...
void habana::BinaryInplaceWrapperOperator::AllocateAndAddSynapseNode(
  ...
  BinaryInplaceOperator binaryInplaceOp(
      this->p_context_->device_id_, guid_, this->scalarType_);

  if (inputs[0].isTensor() && inputs[1].isTensor()) { // Both inputs are tensors
    auto& syn_arg1 =
        binaryInplaceOp.SetSynapseInput(std::move(p_context_->syn_inputs_[0]));
    auto& syn_arg2 =
        binaryInplaceOp.SetSynapseInput(std::move(p_context_->syn_inputs_[1]));
    ...
    auto& syn_arg1 =
        binaryInplaceOp.SetSynapseInput(std::move(p_context_->syn_inputs_[0]));
    UNUSED auto& syn_arg2 =
        binaryInplaceOp.SetSynapseInput(std::move(constOp.GetSynOutputs()[0]));
...
void habana::AddcdivInplaceOperator::AllocateAndAddSynapseNode(
  ...
  // Create Div operator
  habana::DivOperator divOp(this->p_context_->device_id_, scalar_type);
  auto& div_syn_1 =
