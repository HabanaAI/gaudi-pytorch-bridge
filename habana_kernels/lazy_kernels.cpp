...
Tensor remainder_tensor_hpu_lazy(const Tensor& self, const Tensor& other) {
  PT_LAZY_TRACE;
...
Tensor& remainder_tensor_hpu_lazy_(Tensor& self, const Tensor& other) {
  PT_LAZY_TRACE;
...
Tensor& remainder_scalar_hpu_lazy_(Tensor& self, const at::Scalar& other) {
  ...
  LazyOp<Tensor&> k{
      "aten::remainder_", {self, other}, {}, {self.sizes().vec()}};
  return k.call(self);
}

Tensor& remainder_tensor_hpu_lazy_out(
    const Tensor& self,
    const Tensor& other,
    Tensor& result) {
  PT_LAZY_TRACE;
  ...
  if (result.numel() != 0 && result.sizes().vec() != out_shape) {
    ...
    THHTensor_resizeNd(
        out_reshaped, out_shape.size(), out_shape.data(), nullptr);
    result.unsafeGetTensorImpl()->set_sizes_contiguous(IntArrayRef(out_shape));
  }

  LazyOp<Tensor&> k{"aten::remainder", {self, other, result}, {}, {out_shape}};
  return k.call(result);
}
...
Tensor& remainder_scalar_hpu_lazy_out(
  ...
  LazyOp<Tensor&> k{
      "aten::remainder", {self, other, result}, {}, {self.sizes().vec()}};
  return k.call(result);
}
