...
class HabanaOptimizerSparseSgd(torch.nn.Module):
    ...
    def forward(self, indices, valid_count):
        return HabanaOptimizerSparseSgdFunction.apply(self.gradients.to(device),self.weights.to(device),self.moments.to(device), indices, learning_rate, valid_count)
...
print('Weights Output of HabanaOptimizerSparseSgd is on {} device'.format(new_weights.device))
print('Weights Output tensor\n', new_weights.detach().cpu().numpy())
print('Moments Output of HabanaOptimizerSparseSgd is on {} device'.format(new_moments.device))
