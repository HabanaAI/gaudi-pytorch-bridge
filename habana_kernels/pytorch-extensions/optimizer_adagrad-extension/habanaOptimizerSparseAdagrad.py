...
class HabanaOptimizerSparseAdagradFunction(torch.autograd.Function):
    ...
    def forward(ctx,gradients,weights_in,moments_in,indices,learning_rate,valid_count):
        outputs = habanaOptimizerSparseAdagrad_cpp.forward(gradients,weights_in,moments_in,indices,learning_rate,valid_count)
        return outputs
...
class HabanaOptimizerSparseAdagrad(torch.nn.Module):
    ...
    def forward(self, indices, valid_count):
        return HabanaOptimizerSparseAdagradFunction.apply(self.gradients.to(device),self.weights.to(device),self.moments.to(device), indices, learning_rate, valid_count)
...
print('Weights Output of HabanaOptimizerSparseAdagrad is on {} device'.format(new_weights.device))
print('Weights Output tensor\n', new_weights.detach().cpu().numpy())
print('Moments Output of HabanaOptimizerSparseAdagrad is on {} device'.format(new_moments.device))
