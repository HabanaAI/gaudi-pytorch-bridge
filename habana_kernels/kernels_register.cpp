...
void optimizer_adamw_hpu_wrap(
  ...
  runInplaceMaybeWithAccThread(
      "hpu::optimizer_adamw",
      std::move(hpu_op),
      ArrayRef<at::TensorList>{weight_vec, exp_avg_vec, exp_avg_sq_vec});
}
...
TORCH_LIBRARY(hpu, m) {
  ...
  m.def(
      "optimizer_lars(Tensor[] params, Tensor(a!)[] grads, int[] skip_masks, float eeta, float weight_decay, float eps, Tensor lr) -> ()");
  m.def(
