...
void ScatterAddOperator::AllocateAndAddSynapseNode(
  ...
  if (GET_ENV_FLAG_NEW(PT_HPU_USE_UNSORTED_SCATTER_ADD) &&
      HPURegistrar::get_hpu_global_config().getDeterministic() == false &&
...
std::vector<int64_t> get_start_tensor(std::vector<int64_t> h2d_vec) {
  std::vector<int64_t> start(
...
std::vector<int64_t> get_step_tensor(std::vector<int64_t> h2d_vec) {
  std::vector<int64_t> step(
