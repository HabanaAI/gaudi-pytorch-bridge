/******************************************************************************
 * Copyright (C) _r0_r1 HabanaLabs, Ltd.
 * All Rights Reserved.
 *
...
sizes_vec SplitOutputShape(
    UNUSED const at::Stack& stack,
    bool,
    int64_t dim,
    std::vector<int64_t> outshape) {
  TORCH_CHECK(
      outshape[dim] == 3, "Cross: dimension ", dim, " does not have size 3");
  auto split_shape = outshape;
  split_shape[dim] = split_shape[dim] / 3;
  return {split_shape};
}

sizes_vec TransposeOutputShape(
...
void Cross::AddNode(
  ...
  TORCH_CHECK(
      ...
      "Cross: Both tensors should be of same dtype");

  TORCH_CHECK(
      self.sizes().vec() == other.sizes().vec(),
      "Cross: Both tensors should be of same shape");
  ...
  if (stack.at(2).isNone()) {
    ...
    TORCH_CHECK(dim_axis >= 0, "Cross: no dimension of size 3 in input");
  } else {
