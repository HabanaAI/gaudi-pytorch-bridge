...
FALLBACK_CHECK(
    ArgSortStableFallbackCheck,
    const at::Tensor& self,
    bool stable,
    int64_t dim_,
    [[maybe_unused]] bool descending) {
  return shouldFallback(self, stable, dim_);
}
...
void ArgSortStable::AddNode(
    ...
    const at::Stack& stack) {
  auto self = stack.at(0).toTensor();
  auto stable = stack.at(1).toBool();
  auto dim_ = stack.at(2).toInt();
  auto dim = at::maybe_wrap_dim(dim_, self.dim(), /*wrap_scalar=*/true);
  bool descending = stack.at(3).toBool();
  auto k = self.dim() ? self.size(dim) : 1;
