...
void MixtureOfExperts::AddNode(
  ...
  static const std::map<c10::string_view, MoeActivationMode_t>
      activationModeMap = {
  ...
  auto moe_result = OpBackend::BuildNode(
      this,
      graph,
      {get_guid_with_precision("moe", input.scalar_type()),
       std::move(inputs),
       {{input.sizes(), input.scalar_type(), 0}},
       &params,
       sizeof(params)});
...
static const auto& MoEKernelsKernelRegistry = habana::KernelRegistry().add(
